		       ...
		       bool load_all_shares)
{
	...
	DEBUG(0, ("lp_load_ex: refreshing parameters\n"));
	...
	if (lp_config_backend_is_file()) {
		...
		DEBUG(0, ("%s %d, bRetval is %d\n", __FUNCTION__, __LINE__, bRetval));
	} else {
		DEBUG(0, ("Illegal config  backend given: %d\n",
			  lp_config_backend()));
		bRetval = false;
	}
	DEBUG(0, ("111check point!!\n"));
	if (bRetval && lp_registry_shares()) {
		...
		DEBUG(0, ("%s bRetval is %d, load_all_shares:%d\n", __FUNCTION__, bRetval, load_all_shares));
	}
	...
	if (lp_security() == SEC_ADS && strchr(lp_password_server(), ':')) {
		DEBUG(0, ("WARNING: The optional ':port' in password server = %s is deprecated\n",
			  lp_password_server()));
	}
