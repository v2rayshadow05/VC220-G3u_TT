...
#define CFG_PARSE_FILE		"/usr/bin/cfgparse"

#define CFG_FILE_PATH		"/sftp/upload"
...
/* Login first time, for realpath process, if first time login, the default path must be /sftp/upload, 0: not first time login, 1: first time login  */
int firstlogin = 1;
...
{
	if (handle_is_ok(handle, HANDLE_FILE)) {
		unsigned long long wlen = (unsigned long long)handle_bytes_write(handle);
		unsigned long long rlen = (unsigned long long)handle_bytes_read(handle);
		char *file = handle_to_name(handle);
		logit("%s%sclose \"%s\" bytes read %llu written %llu",
		    emsg == NULL ? "" : emsg, emsg == NULL ? "" : " ",
		    file, rlen, wlen);
#if 0		
		if (wlen != 0 && rlen == 0 && (!access(CFG_PARSE_FILE, F_OK|X_OK)) && file != NULL)
		{
			//do the script file here!
			if (strlen(file) <= (sizeof(cmd) - strlen(CFG_PARSE_FILE) - 2))
			{
				int pid = 0;
				sprintf(cmd, "%s %s", CFG_PARSE_FILE, file);
...
#if 0
	if (oldpath[0] != '\0' && oldpath[1] != '\0')
	{
		if (strchr(&oldpath[1], '/') != NULL)
		{
			if (newpath[0] != '\0' && newpath[1] != '\0')
			{
				if (strchr(&newpath[1], '/') != NULL)
				{
					readonly = 0;
				}
				else
...
{
	fd_set *rset, *wset;
	int in, out, max, ch, skipargs = 0, log_stderr = 1;
	...
	log_level = SYSLOG_LEVEL_QUIET;/*log nothing*/
	log_init(__progname, log_level, log_facility, log_stderr);
