...
static int speechDecoder( FILE *fpInput, FILE *fpOutput, USBVM_CODEC_TYPE codecType, int frameSize, BOOL edianChange );
...
/* 
 * fn		static int speechDecoder( FILE *fpInput, FILE *fpOutput, USBVM_CODEC_TYPE codecType, int frameSize, BOOL edianChange )
...
void usbvm_changeFormatToWav ( int endpt, int fileTimeName, BOOL bSleep)
{
	...
	USBVM_CODEC_TYPE codecType = USBVM_CODEC_NULL; 
	FILE *fpWav = NULL;
	FILE *fpRecord = NULL;
...
int usbvm_changePcmToWav(char *pSrcFile, char *pDstFile)
{
	...
	if (access(pDstFile, F_OK) !=0)
	{
		...
		while (fileSizeLeft > 0)
		{
			...
			/* Decoder tranformation for each frame */
			speechDecoder(fpRecord, fpWav, USBVM_CODEC_PCMU, bytesPerFrame, TRUE);
			fileSizeLeft -= bytesPerFrame;
		}
		/* Update wav file header when transformation finished */
		usbvm_updateWavHeader(fpWav);
