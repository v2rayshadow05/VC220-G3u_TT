...
{	
   ...
   USBVM_CODEC_TYPE eptCodec;      /* codec type */
} USBVM_CODEC_ATTRIBUTE;

/* 
 * brief	This table is used to define format attributes of a supported codec
 */
static const USBVM_CODEC_ATTRIBUTE codecAttributeTable[] =
{
   { {24, 30, 8000}, 240, RTP_PAYLOAD_G7231_63,  USBVM_CODEC_G7231_63 },
   { {10, 5,  8000}, 100, RTP_PAYLOAD_G726_16, USBVM_CODEC_G726_16  },
   { {15, 5,  8000}, 150, RTP_PAYLOAD_G726_24, USBVM_CODEC_G726_24  },
   { {20, 5,  8000}, 200, RTP_PAYLOAD_G726_32,  USBVM_CODEC_G726_32  },
   { {25, 5,  8000}, 250, RTP_PAYLOAD_G726_40, USBVM_CODEC_G726_40  },
   { {10, 10, 8000}, 10,  RTP_PAYLOAD_G729A, USBVM_CODEC_G729A    },
   { {40, 5,  8000}, 400, RTP_PAYLOAD_PCMU,  USBVM_CODEC_PCMU     },
   { {40, 5,  8000}, 400, RTP_PAYLOAD_PCMA,  USBVM_CODEC_PCMA     },
   { {0,  0,  0},    0,   RTP_PAYLOAD_NULL,  USBVM_CODEC_NULL     }  /* This line is required to indicate the end of the list */
...
{
   { USBVM_RTP_NTE_DTMF0, '0' },
   { USBVM_RTP_NTE_DTMF1, '1' },
   { USBVM_RTP_NTE_DTMF2, '2' },
   { USBVM_RTP_NTE_DTMF3, '3' },
   { USBVM_RTP_NTE_DTMF4, '4' },
   { USBVM_RTP_NTE_DTMF5, '5' },
   { USBVM_RTP_NTE_DTMF6, '6' },
   { USBVM_RTP_NTE_DTMF7, '7' },
   { USBVM_RTP_NTE_DTMF8, '8' },
   { USBVM_RTP_NTE_DTMF9, '9' },
   { USBVM_RTP_NTE_DTMFS, '*' },
   { USBVM_RTP_NTE_DTMFH, '#' },
   { USBVM_RTP_NTE_DTMFA, 'A' },
   { USBVM_RTP_NTE_DTMFB, 'B' },
   { USBVM_RTP_NTE_DTMFC, 'C' },
   { USBVM_RTP_NTE_DTMFD, 'D' },
...
static void remoteRecThreadCreate( int recordId )
{
	...
	*/
	...
	schedparams.sched_priority = PTHREAD_PRIO_HIGH;
...
static void remoteRecThread( void *context )
{
	...
	while ( sem_wait((sem_t *)&g_usbvm_recordHandle[recordId].semRecord) == -1 && EINTR == errno );
	while (1)
	{
...
{	
	...
	/* close old fp in 'g_usbvm_recordHandle' if it is used,before open new fp */
		...
		while (codecAttributeTable[i].eptCodec != USBVM_CODEC_NULL)
		{
...
void usbvm_remoteRecord_setGotDtmfHashInRecording(int recordId, 
				unsigned short dtmfNum, unsigned short dtmfStartNum, int dtmfType)
{
	if ((USBVM_RTP_NTE_DTMFH == dtmfNum || USBVM_RTP_NTE_DTMFH == dtmfStartNum)
	 ...
	 && g_usbvm_recordHandle[recordId].nGotDtmfHashInRecording < 0)
	{
		...
		TT_PRINT("-----dtmf(%d/%d) dtmfStart(%d/%d) remoteStatus(%d/3) "
			...
			dtmfNum, USBVM_RTP_NTE_DTMFH,
			dtmfStartNum, USBVM_RTP_NTE_DTMFH,
