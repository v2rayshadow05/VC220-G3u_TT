...
#if SET_MARK
#ifdef INCLUDE_SOL
	ret = snprintf(cmd, PJ_ARRAY_SIZE(cmd), "iptables -%c OUTPUT -p %.*s %s %s %s %s -j MARK --set-mark 0x%x",
						act, (int)protoName->slen, protoName->ptr, mark_destination, mark_dportStr, mark_source, mark_sportStr, mark);
#else
	ret = snprintf(cmd, PJ_ARRAY_SIZE(cmd), "iptables -%c OUTPUT -p %.*s %s %s %s %s -j MARK --set-mark 0x61",
