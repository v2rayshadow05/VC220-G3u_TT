...
#ifdef INCLUDE_SOL
/* when pjsip send the register package, send this msg to cmgr, in order to change the status. by wsc. */
/* type enum CMSIP_REG_STATUS */
int cmsip_send_reg(int accId, int type)
{
	CMSIP_MSG cmSipMsg;
	CMSIP_SIP_GENRES* pGenRes = NULL;
	int ret;
	cmSipMsg.type = CMSIP_RESPONSE_SIP_SENDREGISTER;
	pGenRes = (CMSIP_SIP_GENRES*)cmSipMsg.body;
	pGenRes->accId = accId;
	pGenRes->statusCode = type;
	#ifdef CMSIP_DEBUG
	printf("----------%s,%d---------\n", __FUNCTION__, __LINE__);
	printf("accId %d\n", pGenRes->accId);
	printf("type %d\n", pGenRes->statusCode);
	...
	{
		#ifdef CMSIP_DEBUG
		printf("%s,%d: can not send CMSIP_RESPONSE_SIP_SENDREGISTER\n", __FUNCTION__, __LINE__);
		#endif
		return -1;
	}
	 return 0;
...
/* when pjsip sent or received packages which will affect the callState, send msg to cmgr, by wsc. */
/* type: enum CMSIP_SIP_CALL_STATE*/
int cmsip_send_callState(int accId, int type)
{
	CMSIP_MSG cmSipMsg;
	CMSIP_SIP_GENRES* pGenRes = NULL;
	int ret;
	cmSipMsg.type = CMSIP_RESPONSE_SIP_SENDCALLSTATE;
	pGenRes = (CMSIP_SIP_GENRES*)cmSipMsg.body;
	pGenRes->accId = accId;
	pGenRes->statusCode = type;
	#ifdef CMSIP_DEBUG
	...
	{
		#ifdef CMSIP_DEBUG
		printf("%s,%d: can not send CMSIP_RESPONSE_SIP_SENDCALLSTATE\n", __FUNCTION__, __LINE__);
		#endif
		return -1;
	}
	 return 0;
