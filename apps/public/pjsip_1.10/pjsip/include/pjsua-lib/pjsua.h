...
#   define PJSUA_ACC_MAX_PROXIES    1/*8*//*ycw-pjsip*/
	...
	#if defined(NUM_FXS_CHANNELS) && defined(NUM_DECT_CHANNELS)
	#define PJSUA_USBVM_MAX_RECORDID NUM_FXS_CHANNELS + NUM_DECT_CHANNELS
	#elif defined(NUM_FXS_CHANNELS)
...
#	endif
	...
	pj_str_t domain;
	pj_str_t faxCodecList[4];
	unsigned faxCodec_cnt;
	pj_bool_t enableT38;
#endif	
} pjsua_acc_config;
...
struct pjsua_media_config
{
    ...
    /**
     * dingcheng - pjsip .Save the min value for rtp port
     *
     * Default value: PJMEDIA_PORT_LOWER_BOUND
     */
    pj_uint16_t 		rtp_port_lower_bound;

   /**
     * dingcheng - pjsip .Save the max value for rtp port 
     *
     * Default value: PJMEDIA_PORT_UPPER_BOUND
     */
    pj_uint16_t 		rtp_port_upper_bound;
...
#ifdef INCLUDE_SOL
pj_status_t pjsua_media_set_capability(pjsua_media_fmt codec_fmt, pjsua_acc_id acc_id);
#else
...
#if defined(NUM_FXS_CHANNELS) && defined(NUM_DECT_CHANNELS)
#define PJSUA_MAX_CMENDPT NUM_FXS_CHANNELS + NUM_DECT_CHANNELS
#elif defined(NUM_FXS_CHANNELS)
#define PJSUA_MAX_CMENDPT NUM_FXS_CHANNELS
#else
#define PJSUA_MAX_CMENDPT 2
#endif /* NUM_FXS_CHANNELS &&  NUM_DECT_CHANNELS*/
...
typedef struct USBVMENDPTCONFIG
{
	...
	char pin[USB_VOICEMAIL_PINNUM_LEN]; /*the size is the same with CM's*/
} USBVMENDPTCONFIG;
