...
pj_status_t pjsua_media_subsys_init(const pjsua_media_config *cfg)
{
    ...
    status = pjmedia_endpt_create(&pjsua_var.cp.factory, 
				  ...
				  &pjsua_var.med_endpt,
				  pjsua_var.media_cfg.rtp_port_lower_bound);
...
#	endif
	...
	{
		for (i = 0, count = 0; i < acc->cfg.faxCodec_cnt; ++i)
		{	 	
			codec = &acc->cfg.faxCodecList[i];

			if (PJ_TRUE == pjsua_codec_mgr_codec_is_exist(codec))
			{
				if (0 == strncmp(codec->ptr, "t38", strlen("t38")))
				{
					pjsua_codec_set_priority(codec, PJMEDIA_CODEC_PRIO_DISABLED);
					continue;
				}
	...
	for (i = 0; i < PJMEDIA_PORT_TEST_COUNT; ++i)
	{
		...
		CMSIP_PRINT("-------Media port is (%d)----", medport);
			...
			pjmedia_endpt_set_mediaPort(pjsua_var.med_endpt,
									pjsua_var.media_cfg.rtp_port_lower_bound,
									pjsua_var.media_cfg.rtp_port_upper_bound);
		/*test RTP media port*/
