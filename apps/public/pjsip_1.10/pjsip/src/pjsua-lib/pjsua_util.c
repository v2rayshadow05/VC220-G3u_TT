...
/* 
 * brief	check that if one ip in account secure ip list is the same as the matched ip
 */
pj_bool_t pjsua_matchInAccSecureIpList(pjsua_acc_id acc_id, char* matchedIp)
{
	CMSIP_PRINT("----acc(%d), matchedIp(%s)------------", acc_id, matchedIp);
	pjsua_acc *acc = &pjsua_var.acc[acc_id];
	host_ip_node *pnext = NULL;

	/* if Secure IP list is empty , allow any call  */
	if(acc->secure_ip_list  == NULL)
	{
		printf("no any secure call ,so allow all!\n");
		return PJ_TRUE;
	}
	
	for (pnext = acc->secure_ip_list; pnext!=NULL; pnext = pnext->next)
	{
		CMSIP_PRINT("host ip(%s)", pnext->host_ip);
		if (!strcmp(pnext->host_ip, matchedIp))
		{
