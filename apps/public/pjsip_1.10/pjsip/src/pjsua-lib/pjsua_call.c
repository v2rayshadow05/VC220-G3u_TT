...
static pjsua_media_fmt media_capability_setting[PJSUA_MAX_ACC] = {[0 ... (PJSUA_MAX_ACC - 1)] = pjsua_faxr_passthrough};
static pj_status_t pjsua_media_capability_validation(const pjmedia_sdp_session *sdp, int mIndex, pjsua_acc_id acc_id);
...
#ifdef INCLUDE_SOL
	/* not a secure call, return false. by wsc */
	if(acc_id == -2)
	{
		cmsip_send_systemlog(CMSIP_SYSTEMLOG_INFO, "This is a IPcall or is not a secure call from outbound proxy");
		PJSUA_UNLOCK();
		return PJ_TRUE;
	}
#endif
...
#ifdef INCLUDE_SOL
		if (pj_stricmp(&(offer->media[0]->desc.media), &image) == 0 &&  !pjsua_var.acc[acc_id].cfg.enableT38)
#else
...
#	if defined(SUPPORT_FAX_T38) && SUPPORT_FAX_T38!=0
#ifdef INCLUDE_SOL
	if (pjsua_var.acc[acc_id].cfg.enableT38)
#else
	if (pjsua_var.enableT38)
#endif /* INCLUDE_SOL */
	{
...
#	if defined(SUPPORT_FAX_T38) && SUPPORT_FAX_T38!=0
#ifdef INCLUDE_SOL
	if (pjsua_var.acc[acc_id].cfg.enableT38 && (options & PJSIP_INV_ENABLE_T38) == 0)
#else
...
#	endif
	...
	call->inv->prevFaxCodec |= INV_PREV_FAX_PASSTHROUGH;
    pjsip_dlg_dec_lock(dlg);
										...
										const pjsua_msg_data *msg_data)
{
	...
	if (pj_channel_t38 == mode)
	{
		...
		call->inv->prevFaxCodec |= INV_PREV_FAX_T38;
	}
...
#ifdef INCLUDE_SOL
		    (media_capability_setting[acc_id] & pjsua_faxr_passthrough) ? "Y" : "N",
                    (media_capability_setting[acc_id] & pjsua_faxr_t38) ? "Y" : "N"));
#else
...
#ifdef INCLUDE_SOL
    media_capability_setting[acc_id] = codec_fmt;
#else
...
#ifdef INCLUDE_SOL
			if (media_capability_setting[acc_id] & pjsua_faxr_t38) /* nothing */;
#else
