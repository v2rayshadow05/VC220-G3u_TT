...
#ifdef INCLUDE_SOL
	dst->faxCodec_cnt = src->faxCodec_cnt;
	for (i=0; i<src->faxCodec_cnt; ++i)
		pj_strdup_with_null(pool, &dst->faxCodecList[i], &src->faxCodecList[i]);
	dst->enableT38 = src->enableT38;
#endif /* INCLUDE_SOL */
}
...
#if  defined(INCLUDE_SOL)
	char outbound_proxy[MAX_URI_LEN];
	struct hostent *secureIPList = NULL;
	pjsip_name_addr *outbound_addr;
#endif	
...
#	endif
	...
	for (i=0; i<acc_cfg->proxy_cnt; ++i)
	{
		...
		/* dingcheng-pjsip:save OutBoung proxy IP */
#if  defined(INCLUDE_SOL)
		outbound_addr = (pjsip_name_addr*)
		    pjsip_parse_uri(acc->pool, acc_cfg->proxy[i].ptr,
				    acc_cfg->proxy[i].slen, 
				    PJSIP_PARSE_URI_AS_NAMEADDR);
		...
		}
		{
			...
			/* Get the SIP URI object: */
			sip_uri = (pjsip_sip_uri*) pjsip_uri_get_uri(outbound_addr);

			/* Save the user and domain part. These will be used when finding an
			 * account for incoming requests.
			 */
			sprintf(outbound_proxy, "%.*s", (int)sip_uri->host.slen, sip_uri->host.ptr);
		
		}
		
	  
		printf("the outbound proxy is :%s \n",outbound_proxy);
		secureIPList= gethostbyname(outbound_proxy);
		if (secureIPList)
		{
			int j;
			
			host_ip_node* pNode = NULL;
			host_ip_node** ppNext = &acc->secure_ip_list;

			
			for (j = 0; (secureIPList->h_addr_list)[j] != NULL; ++j)
			{
				
				pNode = PJ_POOL_ZALLOC_T(acc->pool, host_ip_node);
				inet_ntop(AF_INET, (secureIPList->h_addr_list)[j],
					pNode->host_ip, sizeof(pNode->host_ip));
				cmsip_send_systemlog(CMSIP_SYSTEMLOG_INFO, 
						"get trusted ip(%s) for domain%d (%s) ", pNode->host_ip, i ,outbound_proxy
						);
				*ppNext = pNode;
				ppNext = &pNode->next;
			}
			...
			*ppNext = NULL;
			/*save the ob to endpt. by wsc*/
			for(pNode = acc->secure_ip_list;pNode!=NULL;pNode=pNode->next)
			{
				pjsip_endpt_secureIPList_addItem(pjsua_var.endpt, pNode->host_ip);
			}
		}
#endif
				   ...
				   pjsua_acc_id *p_acc_id)
{
	...
	if (cfg->enableT38)
	{
		pjsua_media_set_capability(pjsua_faxr_t38, id);
	}
#endif /* INCLUDE_SOL */
...
#ifdef INCLUDE_SOL
		cmsip_send_reg(acc->cmAcctIndex, 3);
#endif /* INCLUDE_SOL */
...
#ifdef INCLUDE_SOL
		cmsip_send_reg(acc->cmAcctIndex,(renew? 1 : 2 ));
#endif /* INCLUDE_SOL */
...
#ifdef INCLUDE_SOL
		{
			cmsip_send_systemlog(CMSIP_SYSTEMLOG_INFO, "IP call comes : From IP (%s)", 
...
#	endif
		...
		strAorUri.username, strAorUri.host, strAorUri.port);
						...
						if (!strcmp(strAorUri.username, strReqUri.username)
#if  defined(INCLUDE_SOL)
						&& pjsua_matchInAccSecureIpList(tmp_acc_id, rdata->pkt_info.src_name)
#endif
						)
						{
							if ( acc_id < 0 )
...
#if  defined(INCLUDE_SOL)
				cmsip_send_systemlog(CMSIP_SYSTEMLOG_INFO, "Unsafe call comes : To acc (%d) From IP (%s)", 
						tmp_acc_id , rdata->pkt_info.src_name);
