...
#define FORBIDDEN_LIST_MAXSIZE 8
#endif
/**
 * The SIP endpoint.
 */
...
#if  defined(INCLUDE_SOL)
    /* secure IP list for reject all other packages, by wsc*/
    char		secure_list[FORBIDDEN_LIST_MAXSIZE][MAX_IPADDR_LEN];
    int 		securelist_num;
#endif	
...
#if  defined(INCLUDE_SOL)
/*
 * add item into forbidden IP list.
 */
pj_status_t pjsip_endpt_secureIPList_addItem(pjsip_endpoint *endpt,char * secureIP)
{
	if (endpt->securelist_num >= FORBIDDEN_LIST_MAXSIZE)
	{
		cmsip_send_systemlog(CMSIP_SYSTEMLOG_INFO, "the secure IP list is full, add IP (%s) failed", 
						secureIP);
		return (-1);
	}
	strcpy(endpt->secure_list[endpt->securelist_num],secureIP);

	cmsip_send_systemlog(CMSIP_SYSTEMLOG_INFO, "add IP (%s) into the secure IP list ", 
						secureIP);
	
	endpt->securelist_num++;
...
/*
 * check the forbidden IP list to match the IP
 */
pj_status_t pjsip_endpt_secureIPList_checkItem(pjsip_endpoint *endpt,char * checkedIP)
{
	int i;
	if(endpt->securelist_num==0)
	{
		return PJ_TRUE;
	}
	for(i = 0 ; i < endpt->securelist_num; i++)
	{
		if(!strcmp(endpt->secure_list[i],checkedIP))
		{
...
#endif /* defined(INCLUDE_SOL) && INCLUDE_SOL!=0 */
				      ...
				      pjsip_rx_data *rdata )
{
	...
	return;
	 ...
	 {
		/* wsc:check whether the packet is in secure IP list */
		if(!pjsip_endpt_secureIPList_checkItem(endpt, rdata->pkt_info.src_name))
		{
			cmsip_send_systemlog(CMSIP_SYSTEMLOG_INFO, "Forbidden IP (%s) sends a sip request here and has been discard", 
							rdata->pkt_info.src_name);
			return ;
		}
	 }
