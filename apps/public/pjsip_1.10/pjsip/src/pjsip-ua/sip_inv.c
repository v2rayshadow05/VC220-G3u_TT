						  ...
						  pjsip_rx_data *rdata)
{
	...
	/*ycw-pjsip-enableT38*/
		...
		if (inv->neg && inv->neg->neg_local_sdp)
		{
			unsigned options = 0;
...
#	endif
			...
			struct pjsua_data *ppjsua_var = pjsua_get_var();
			
			if (inv->dlg && inv->dlg->mod_data[ppjsua_var->mod.id])
				acc_id = ((pjsua_call*) inv->dlg->mod_data[ppjsua_var->mod.id])->acc_id;
			if (acc_id == -1)
			{
...
#	if defined(SUPPORT_IPCALL_NO_ACCOUNT) && SUPPORT_IPCALL_NO_ACCOUNT!=0
			else if (PJSIP_INVITE_METHOD == rdata->msg_info.msg->line.req.method.id)
			{
				pjsua_isForwardCall(rdata->msg_info.msg, &ipCall);
			}
#	endif
			/* Verify that we can handle the request. */
			options |= PJSIP_INV_SUPPORT_100REL;
...
#	if defined(SUPPORT_IPCALL_NO_ACCOUNT) && SUPPORT_IPCALL_NO_ACCOUNT!=0
			if (ipCall)
			{
				if (ppjsua_var->ua_cfg.require_100rel)
			 	{
			 		options |= PJSIP_INV_REQUIRE_100REL;
			 	}
			 
#if	PJSUA_ADD_ICE_TAGS
		    	if (ppjsua_var->media_cfg.enable_ice)
		    	{
					options |= PJSIP_INV_SUPPORT_ICE;
		    	}
#endif	 
		    	if (ppjsua_var->ua_cfg.use_timer == PJSUA_SIP_TIMER_REQUIRED)
		    	{
					options |= PJSIP_INV_REQUIRE_TIMER;
		    	}
		    	else if (ppjsua_var->ua_cfg.use_timer == PJSUA_SIP_TIMER_ALWAYS)
		    	{
			...
			{
#	endif
				if (ppjsua_var->acc[acc_id].cfg.require_100rel)
			 	{
			 		options |= PJSIP_INV_REQUIRE_100REL;
			 	}
...
#endif	 
		    	if (ppjsua_var->acc[acc_id].cfg.use_timer == PJSUA_SIP_TIMER_REQUIRED)
		    	{
					options |= PJSIP_INV_REQUIRE_TIMER;
		    	}
		    	else if (ppjsua_var->acc[acc_id].cfg.use_timer == PJSUA_SIP_TIMER_ALWAYS)
		    	{
					options |= PJSIP_INV_ALWAYS_USE_TIMER;
		    	}
...
#	if defined(SUPPORT_FAX_T38) && SUPPORT_FAX_T38!=0
#ifdef INCLUDE_SOL
			if (acc_id != -1 && ppjsua_var->acc[acc_id].cfg.enableT38)
#else
			if (ppjsua_var->enableT38)
#endif /* INCLUDE_SOL */
			{
				options |= PJSIP_INV_ENABLE_T38;
			}
#	endif
			status = pjsip_inv_verify_request2(rdata, &options, sdp_info->sdp, inv->neg->neg_local_sdp, NULL,
						       ppjsua_var->endpt, &response);
			if (status != PJ_SUCCESS)
			{
				CMSIP_PRINT("-----on_rx_offer return %d-------", status);
				/* reset state to PJMEDIA_SDP_NEG_STATE_REMOTE_OFFER */
    			inv->neg->state = PJMEDIA_SDP_NEG_STATE_REMOTE_OFFER;
				return status;
			}
		}
	    ...
	    if (tsx == inv->invite_tsx)
	    {
		 ...
		 if (tsx->state < PJSIP_TSX_STATE_TERMINATED)
		 {
			 switch (inv->holdStat)
			 {
				 ...
				 break;
				 case PJSIP_HOLD_T38:
				 	/* negotiated T38 with peer failed, inform cm and cm will tell sip to try passthrough */
					if (!(inv->prevFaxCodec & INV_PREV_FAX_PASSTHROUGH))
					{
				 		cmsip_send_callStatus(CMSIP_CALL_T38, inv->dlg->callIndex, inv->dlg->seq,  
											  tsx->status_code, contactArray, 1);		
					}
				 break;
				 case PJSIP_HOLD_PASSTHROUGH:
				 	/* negotiated G.711 with peer failed, inform cm and cm will tell sip to try T.38 */
					if (!(inv->prevFaxCodec & INV_PREV_FAX_T38))
					{
				 		cmsip_send_callStatus(CMSIP_CALL_PSTN, inv->dlg->callIndex, inv->dlg->seq,
											  tsx->status_code, contactArray, 1);
					}
