...
CMSIP_PRINT("-----notify stream Info to CM!");
	...
	/* If the fmpt params specify "annexb = no", we should tell cm use 729a rather than 729 */
	/* SDP just knows G.729 and doesn't care about A/B, it maybe ugly to put here */
	if (stm->fmt.pt == PJMEDIA_RTP_PT_G729)
	{
		int index = 0;
		for (index = 0; index < stm->param->setting.enc_fmtp.cnt; index++)
		{
			if (0 == pj_stricmp2((const pj_str_t *)&(stm->param->setting.enc_fmtp.param[index].name), "annexb"))
			{
				if (0 == pj_stricmp2((const pj_str_t *)&(stm->param->setting.enc_fmtp.param[index].val), "no"))
				{
					strcpy(codecName, "G729A");
				}
				break;
			}
		}
