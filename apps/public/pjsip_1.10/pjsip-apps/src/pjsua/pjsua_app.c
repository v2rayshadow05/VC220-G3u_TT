...
/*ycw-pjsip-enableT38*/
	   ...
	   /*USB Voice Mail*/
		...
		OPT_PREFER_ID,
#endif
		/* dingcheng -pjsip.add rtp port  options*/
		OPT_RTP_MIN_PORT,
		OPT_RTP_MAX_PORT,
#ifdef INCLUDE_SOL
		OPT_SKB_MARK,
...
#endif
		/* dingcheng-pjsip */
		{"rtp-min-port",				1,	0,	OPT_RTP_MIN_PORT},
		{"rtp-max-port",				1,	0,	OPT_RTP_MAX_PORT},
#ifdef INCLUDE_SOL
		{ "set-mark",                   1,  0,  OPT_SKB_MARK},
		{ "fax-codec",					1,	0,	OPT_FAX_CODEC},
#endif /* INCLUDE_SOL */
		{ NULL, 								0, 0, 0}
    };
    pj_status_t status;
...
#if  defined(INCLUDE_SOL)
	    cur_acc->domain = pj_str(pj_optarg);
#endif			 
...
/*ycw-pjsip*/
	...
	case OPT_FWTYPE:
		...
		break;
	/* dingcheng-pjsip */
	case OPT_RTP_MIN_PORT:
		{
			cfg->media_cfg.rtp_port_lower_bound = (pj_uint16_t)my_atoi(pj_optarg);
			cfg->rtp_cfg.port = cfg->media_cfg.rtp_port_lower_bound;
		}
		break;
	case OPT_RTP_MAX_PORT:
		{
			cfg->media_cfg.rtp_port_upper_bound = (pj_uint16_t)my_atoi(pj_optarg);
		}
		break;
#ifdef INCLUDE_SOL		
	case OPT_SKB_MARK:
		{
			pj_str_t s;
			pj_cstr(&s, pj_optarg);
			cfg->mark = (pj_uint32_t)pj_strtoul2(&s, NULL, 16);
		}
		break;
		
	case OPT_FAX_CODEC:		
		if (cur_acc->enableT38 == PJ_FALSE && 0 == strcmp(pj_optarg, "t38"))
			cur_acc->enableT38 = PJ_TRUE;
		cur_acc->faxCodecList[cur_acc->faxCodec_cnt++] = pj_str(pj_optarg);
...
#	endif
	...
	if (acfg->auth_pref.initial_auth && acfg->cred_count) {
		...
		/* dingcheng-pjsip:as some error comes,disable this function */
		acfg->auth_pref.initial_auth = PJ_FALSE;
		continue;
...
#ifdef INCLUDE_SOL
	pjsua_var.mark = app_config.mark;
#endif /* INCLUDE_SOL */
...
#if defined(INCLUDE_SOL)
                /*in SOL environment, too many requests in a short time will cause deny */
