...
int cmmlog_logRemoute(unsigned int ip, unsigned short port, const char *content, unsigned int len)
{
	int sock;
	...
	err = sendto(sock, (void *)content, len, 0, (struct sockaddr *)&addr, sizeof(addr));
	if (err < 0)
	{
		err = -errno;
		printf("[ %s ]%d: Sending log %s error: %s", __FUNCTION__, __LINE__, content, strerror(errno));
	}
	close(sock);
