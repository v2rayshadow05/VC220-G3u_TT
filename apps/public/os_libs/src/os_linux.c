...
/*                                      DEFINES                                                   */
/**************************************************************************************************/
#define MAX_TIMER_NUM	128/*(SIGNUM_MAX-SIGNUM_MIN)*//*32*//*If we define MAX_TIMER_NUM as 32, this may 
														result in timers array overflow*/
...
/* 
 * fn		int os_semGetValue(OS_SEM *pSem)
 * brief	places the current value of the semaphore pointed to sem into the integer pointed to by sval. 
 * details	This routine performs the get operation on a specified semaphore.
 *
 * param [in]	sem - semaphore ID to give 
 * param[out]	
 *
 * return	0 if success, -1 otherwise.		
 */
int os_semGetValue(OS_SEM *pSem, int *sval)
{
    if (0 != sem_getvalue(pSem, sval))
    {
		perror("sem_getvalue");
        printf("Semaphore getvalue failed.");
        return -1;
    }
