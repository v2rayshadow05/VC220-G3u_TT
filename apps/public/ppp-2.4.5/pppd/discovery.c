...
discovery(PPPoEConnection *conn)
{
	...
	/* 每次启动pppd进程，都尝试发PADT包终结上次的session. Added by Wang Jianfeng 2014-05-05*/
	/* adapt it for sending PADT without any session before , fixed by caoqichen */
	...
	padiAttempts++;
#ifdef INCLUDE_SOL
	if (padiAttempts > 1) { /* for pppoe connection delay */
#else
 ...
 #ifdef INCLUDE_WORK_AROUND
	if(strstr(conn->ifName,"nas8")==NULL)
	{
		//printf("-------we are in the ad mode,set the flag!----------\n");
		char pppstatuscmd[40];
		sprintf(pppstatuscmd,"echo 1 >/var/tmp/CPE_WanStat/%s",myPPPoEName);
		system(pppstatuscmd);
	}
#endif
