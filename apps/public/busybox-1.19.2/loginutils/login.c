...
#ifdef INCLUDE_TTNET
#include "os_msg.h"
#define CONSOLE_KEY_FILE	"/var/tmp/cskey"
#endif
...
int login_main(int argc UNUSED_PARAM, char **argv)
{
	...
	/*added by xianjiantao for 9970 ttnet 3.0,17-11-20*/
#ifdef INCLUDE_TTNET
	/*get the root username and password from dm.always make the console login username and password
	follow the dm*/
	CMSG_FD 	msgFd = {0};
	CMSG_BUFF 	msgBuff = {0};
	char realusername[USERNAME_SIZE];
	char *pwdInput;
	char bAuthSuccess = 0;
	int fd;
	md5_ctx_t md5;
	char *md5line, *cal_md5line, *buf;
#endif
	...
	while (1) {
		...
		/*added by xianjiantao for 9970 ttnet 3.0,17-11-20*/
#ifdef INCLUDE_TTNET
		safe_strncpy(realusername, username, sizeof(realusername));
		safe_strncpy(username, "admin", sizeof(username));			
#endif
		/*added end xianjiantao*/
 ...
 fake_it:
		...
		/*modified by xianjiantao for 9970 ttnet 3.0,17-11-20*/
#ifdef INCLUDE_TTNET
		pwdInput = bb_ask_stdin("Password: ");
		if (pw && pwdInput)
		{	
			/*read the origin md5*/
			md5line = xmalloc(16 * 2 + 1);
			fd = open(CONSOLE_KEY_FILE, O_RDONLY);
			if (fd >= 0) {
				read(fd, md5line, 16 * 2 + 1);
				close(fd);
			}
			
			/*calculate the input info md5*/
			buf = xmalloc(USERNAME_SIZE + 128 + 1);/*username:pw*/
			sprintf(buf, "%s:%s", realusername, pwdInput);
			md5_begin(&md5);
			md5_hash(&md5, buf, strlen(buf));
			md5_end(&md5, buf); /* using buf as result storage */

			cal_md5line = xmalloc(16 * 2 + 1);
			bin2hex(cal_md5line, buf, 16);

			if (0 == memcmp(md5line, cal_md5line, 16 * 2))
				bAuthSuccess = 1;
			
			free(buf);
			free(cal_md5line);
			free(md5line);
			memset(pwdInput, 0, strlen(pwdInput));

			/*compare the calculate md5 and origin md5*/
			if (bAuthSuccess)
				break;
		}
 ...
 auth_failed:
	...
	msgBuff.type = CMSG_SET_CONSOLE_PID;
	msgBuff.priv = getpid();
