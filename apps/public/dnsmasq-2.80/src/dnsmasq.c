...
int main (int argc, char **argv)
{
  ...
  if (option_bool(OPT_ADD_MAC))
    daemon->pppUsername = safe_malloc(PPP_USERNAME_LEN);
	...
	timeout = t * 1000;
	  ...
	  (option_bool(OPT_DBUS) && !daemon->dbus)
		/*modified by xianjiantao for c5v tt,17-12-07*/
	  	/*set the timeout to 250ms to see if a frec need to go to next stage frequently*/
...
#    endif
      ...
      if (is_dad_listeners())
	{
	  ...
	  if (option_bool(OPT_ADD_MAC))
	{
		if (daemon->timestamp == 0 || 
	 	  difftime(now, daemon->timestamp) > 1.0 || 
	 	  difftime(now, daemon->timestamp) < -1.0)
	 	{
	      updatePPP();  
...
#ifdef INCLUDE_ENHANCE_DNS_PROXY
	/*check the frec list here to see if a frec need to go to next stage*/
	check_frec_list(now);
#endif
