...
{
	FILE *fp = NULL;
	char buff[PPP_USERNAME_LEN] = {0};

	if (!(fp = fopen("/tmp/pppoe", "r")))
	{
		my_syslog(LOG_ERR, _("failed to read /tmp/pppoe: %s"), strerror(errno));
		return;
	}

	if (NULL != fgets(buff, PPP_USERNAME_LEN, fp))
	{
		char *token = strtok(buff, " \t\n\r");
		strcpy(daemon->pppUsername, token);
	}
	fclose(fp);
	return;
}

static size_t add_ppp(struct dns_header *header, size_t plen, unsigned char *limit, union mysockaddr *l3, time_t now)
{
	int len = strlen(daemon->pppUsername);

	if (len)
	{
		unsigned char ppp[PPP_USERNAME_LEN + 2];
		memcpy(ppp + 2, daemon->pppUsername, len);
		ppp[0] = 0x00;
		ppp[1] = 0x20;
		plen = add_pseudoheader(header, plen, limit, PACKETSZ, EDNS0_OPTION_PPPOE, ppp, len + 2, 0, 0);
	}
...
static size_t add_mac(struct dns_header *header, size_t plen, unsigned char *limit, union mysockaddr *l3, time_t now)
{
	...
	while (0 == (maclen = find_mac(l3, mac, 1, now)) && loop < 10)
	{
		loop++;
		sleep(1);
	}
	...
	if (maclen != 0) 
	{	
		unsigned char mac2[DHCP_CHADDR_MAX + 2];
		memcpy(mac2 + 2, mac, maclen);
		mac2[0] = 0x40;
		mac2[1] = 0x05;
		plen = add_pseudoheader(header, plen, limit, PACKETSZ, EDNS0_OPTION_MAC, mac2, maclen + 2, 0, 0); 
	}
	else
	{
		//for debug
		system("arp > /tmp/arp_log");
	}
			...
			union mysockaddr *source, time_t now, int *check_subnet)    
{
  *check_subnet = 0;
  ...
  if (option_bool(OPT_ADD_MAC))
  	{
  	  plen  = add_ppp(header, plen, limit, source, now);
