			 ...
			 struct frec *forward, int ad_reqd, int do_bit)
{
...
#ifdef INCLUDE_ENHANCE_DNS_PROXY
  struct timeval time;
#endif
  ...
  /*modified by xianjiantao for c5v tt,17-12-07*/
#ifdef INCLUDE_ENHANCE_DNS_PROXY
	/*in this case,we are running a state machine for this frec already and if we receive the same client sending the same
	query with the same transaction id,just ignore it*/
  ...
  /*modified end xianjiantao*/
      ...
      if (!option_bool(OPT_ORDER))
	{
      ...
      /*modified by xianjiantao for c5v tt,17-12-07*/
#ifdef INCLUDE_ENHANCE_DNS_PROXY
	  if (forward->state >= FREC_STATE_5)
	  {
			return 0;
	  }
      if (FREC_STATE_1 == forward->state)
       start = forward->sentto;
      else if (!(start = forward->sentto->next))
	start = daemon->servers;

      if (FREC_STATE_2 == forward->state || FREC_STATE_3 == forward->state)
       forward->forwardall = 1;
#else   
	...
	flags = search_servers(now, &addrp, gotname, daemon->namebuff, &type, &domain, &norebind);
	  ...
	  plen = add_edns0_config(header, plen, ((unsigned char *)header) + PACKETSZ, udpaddr, now, &subnet);
      ...
      /*modified by xianjiantao for c5v tt,17-12-07*/
#ifdef INCLUDE_ENHANCE_DNS_PROXY
			forward = get_new_frec(now, plen,NULL, 0);	
#else
      ...
      if (forward)
	{
	  ...
	  /*modified by xianjiantao for c5v tt,17-12-07*/
#ifdef INCLUDE_ENHANCE_DNS_PROXY
        memcpy(forward->packet, header, forward->len);
#endif
        /*modified end xianjiantao*/
	...
	{ 
	  ...
	  if (type == (start->flags & SERV_TYPE) &&
	      ...
	      if (errno == 0)
		{
		  ...
		  start->queries++;
		  forwarded = 1;
		  /*added by xianjiantao for c5v tt,17-12-07*/
#ifdef INCLUDE_ENHANCE_DNS_PROXY
		  gettimeofday(&time,NULL);
		  memcpy(&forward->forwardtime,&time,sizeof(time));
#endif	  		  
...
void reply_query(int fd, int family, time_t now)
{
  ...
  /* If the answer is an error, keep the forward record in place in case
     ...
     /*modified by xianjiantao for c5v tt,17-12-07*/
#ifdef INCLUDE_ENHANCE_DNS_PROXY
     if (((forward->forwardall == 0 || --forward->forwardall == 1) && forward->state >= FREC_STATE_3) || 
...
/* if wait==NULL return a free or older than TIMEOUT record.
   ...
   /*modified by xianjiantao for c5v tt,17-12-07*/
#ifdef INCLUDE_ENHANCE_DNS_PROXY
int check_diff_time(struct timeval *time_now,struct timeval *time_last)
{
	long long us_diff = 0;
	us_diff = ((time_now->tv_sec - time_last->tv_sec) * 1000000 + time_now->tv_usec) - time_last->tv_usec + 50000;
	return (int)(us_diff/1000000);
}
void check_frec_list(time_t now)
{
	struct frec *f;
	struct timeval time_now;
	gettimeofday(&time_now,NULL);
	/*polling the frec list*/
	for(f = daemon->frec_list; f; f = f->next)
		if (f->sentto)
		{
			switch (f->state)
			{
				case FREC_STATE_0:
				case FREC_STATE_1:
					if (check_diff_time(&time_now, &f->forwardtime) >= 1)
						{
							forward_query(-1, NULL, NULL, 0, f->packet, f->len, now, f,f->flags & FREC_AD_QUESTION, f->flags & FREC_DO_QUESTION);
							f->state++;
						}
				break;

				case FREC_STATE_2:
					if (check_diff_time(&time_now, &f->forwardtime) >= 2)
						{
							forward_query(-1, NULL, NULL, 0, f->packet, f->len, now, f,f->flags & FREC_AD_QUESTION, f->flags & FREC_DO_QUESTION);
							f->state++;
						}
				...
				case FREC_STATE_3:
					if (check_diff_time(&time_now, &f->forwardtime) >= 4)
						{
							forward_query(-1, NULL, NULL, 0, f->packet, f->len, now, f,f->flags & FREC_AD_QUESTION, f->flags & FREC_DO_QUESTION);
							f->state++;
						}
				...
				default:
				/*what should we do here?free the frec?*/
				break;
			}
		}
...
static int realloc_frec_buf(struct frec *f, size_t size)
{
	char *tmp = NULL;
	...
	/*which means the buff now is enough and no need to realloc*/
	if (size <= f->bufLen && f->packet)
	{
		f->len = size;
		return 1;
	}
	...
	{
		/*try malloc first to ensure that if the malloc is failed,the current f->packet won't be free*/
		if (!(tmp = (char *)whine_malloc(size)))
		{
			return 0;
		}
...
#ifdef INCLUDE_ENHANCE_DNS_PROXY
struct frec *get_new_frec(time_t now, size_t pktSize, int *wait, int force)
#else
  ...
  if (target)
    {
      /*added by xianjiantao for c5v tt,17-12-07*/
#ifdef INCLUDE_ENHANCE_DNS_PROXY
      if (!realloc_frec_buf(target, pktSize))
	  	return NULL;
#endif
      /*added end xianjiantao*/
      target->time = now;
  ...
  if (!force && oldest && ((int)difftime(now, oldest->time)) >= TIMEOUT)
    { 
	 ...
	 record instead */
	  ...
	  /*added by xianjiantao for c5v tt,17-12-07*/
#ifdef INCLUDE_ENHANCE_DNS_PROXY
/*doesn't matter if the allocate_frec succeed while realloc_frec_buf don't,because the idle frec will always be recycled*/
           if (!realloc_frec_buf(f, pktSize))
	  	return NULL;
#endif
/*added by xianjiantao for c5v tt,17-12-07*/
		return f;
      ...
      if (!wait)
	{
	  /*added by xianjiantao for c5v tt,17-12-07*/
#ifdef INCLUDE_ENHANCE_DNS_PROXY
         if (!realloc_frec_buf(oldest, pktSize))
	  	return NULL;
#endif
      	  /*added end xianjiantao*/
	  free_frec(oldest);
