...
int main (int argc, char** argv)
{
...
#ifdef INCLUDE_TTNET
	UPNP_ENTRY_ENABLE_MSG *pUpnpEnEntryMsg = NULL;
#endif
	...
	while(arg < argc)
	{
		...
		/*modified by xianjiantao for vr600 tt v1 11 ac new requirement issue 18,17-04-12*/
#ifdef INCLUDE_TTNET
		...
		else
		{
			...
			printf("Usage: \n"\
				   "upnpd [-F] -L <internal ifname> -W <external ifname>"\
				   "-en <enable>  -nat <enable> -map <enable> -url <manufacterUrl> -ma <manufacter>"\
				   "-mn <moduleName> -mv <modelVer> -desc <description>");
#endif
			/*modified end xianjiantao*/
			...
			if (FD_ISSET(fdUPnP.fd, &fds))
       		{
				...
				{
					g_vars.portMappingEnabled = msgBuff.priv;
				}
				else if (msgBuff.type == CMSG_UPNP_DEL_ALL_ENTRY)
				{
					DeleteAllPortMappings();
				}
				else if (msgBuff.type == CMSG_UPNP_ENABLE_ENTRY)
				{
					pUpnpEnEntryMsg = (UPNP_ENTRY_ENABLE_MSG *)(msgBuff.content);
					enableSpecificRule(pUpnpEnEntryMsg->enable, pUpnpEnEntryMsg->port,
										pUpnpEnEntryMsg->protocol);
				}
#endif
				/*added end xianjiantao*/
...
#else
			"  -nat %d  -map %d -port %d  -url  %s  -ma  %s  -mn  %s  -mv  %s  -desc  %s&\n",			 
#endif
			 /*modified end xianjiantao*/
			 "br0",
