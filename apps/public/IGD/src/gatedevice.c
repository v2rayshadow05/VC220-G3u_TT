...
/*added by xianjiantao for vr600tt,17-05-04*/
#ifdef INCLUDE_TTNET
static void notifyCosRulesUpdate();
#endif
...
int HandleActionRequest(struct Upnp_Action_Request *ca_event)
{
	...
	if (strcmp(ca_event->DevUDN, gateUDN) == 0)
	{
		...
		else if (strcmp(ca_event->ServiceID, "urn:upnp-org:serviceId:WANIPConn1") == 0)
		{
			...
			/*modified by xianjiantao for vr600tt,17-05-04*/
			/*for ttnet's new requirement.all of the portmapping rules,including upnp rules,
			should be seen via ACS*/
...
#ifdef INCLUDE_TTNET
			  if (UPNP_E_SUCCESS == ca_event->ErrCode)
			  	notifyCosRulesUpdate();
			}
#endif
			/*modified end xianjiantao*/
...
int AddPortMapping(struct Upnp_Action_Request *ca_event)
{
	...
	int ext_port_num=0;
	int ftp_ctrl_port=0;
	/*added by xianjiantao fot 9970 ttnet 3.0,17-05-25*/
#ifdef INCLUDE_TTNET
	int httpRemotePort;
	int acsPort = 0;
#endif
	     ...
	     && (desc = GetFirstDocumentItem(ca_event->ActionRequest, "NewPortMappingDescription") ))
	{
		...
		/*added by xianjiantao fot 9970 ttnet 3.0,17-05-25*/
#ifdef INCLUDE_TTNET
		if((proto == NULL) || ((strcmp(proto,"UDP") != 0) && (strcmp(proto,"udp") != 0)))
		{
			fp = fopen(HTTP_REMOTE_PORT_FILE, "r");
			if (fp == NULL)
			{
				trace(1, "Failiure in GateDeviceAddPortMapping: Open file error:%d", errno);
				ca_event->ErrCode = errno;
			...
			httpRemotePort = atoi(bufFile);
			if (httpRemotePort == ext_port_num)
			{
				trace(1, "Failiure in GateDeviceAddPortMapping: Port conflict with HTTP Remote Access!");
			    trace(1, "  ExtPort: %s Proto: %s IntPort: %s IntIP: %s Dur: %s Ena: %s Desc: %s",
				ext_port, proto, int_port, int_ip, int_duration, bool_enabled, desc);
			    ca_event->ErrCode = 402;
			    strcpy(ca_event->ErrStr, "Invalid Args");
			...
			acsPort = atoi(bufFile);
			if (acsPort == ext_port_num)
			{
				trace(1, "Failiure in GateDeviceAddPortMapping: Port conflict with HTTP Remote Access!");
			    trace(1, "  ExtPort: %s Proto: %s IntPort: %s IntIP: %s Dur: %s Ena: %s Desc: %s",
				ext_port, proto, int_port, int_ip, int_duration, bool_enabled, desc);
...
int DeletePortMapping(struct Upnp_Action_Request *ca_event)
{
   ...
   if (action_succeeded)
   {
      ...
      snprintf(resultStr, RESULT_LEN, "<u:%sResponse xmlns:u=\"%s\">\n%s\n</u:%sResponse>",
         ca_event->ActionName, "urn:schemas-upnp-org:service:WANIPConnection:1", "", ca_event->ActionName);
      ca_event->ActionResult = ixmlParseBuffer(resultStr);
	/*added by xianjiantao for vr600tt,17-05-05*/
#ifdef INCLUDE_TTNET
	notifyCosRulesUpdate();
#endif
	/*added end xianjiantao*/
...
/*added by xianjiantao for vr600tt,17-05-04*/
#ifdef INCLUDE_TTNET
static void updateRulesToFile()
{
	/*this function should only be called when you acquire the DevMutex lock!*/
	FILE *pFWriter;
	int pmIndex;
	...
	/*don't try to get the DevMutex lock here!when this func is called,that lock has been acquired.*/
	...
	if (NULL != pFWriter)
	{
		...
		truncate(UPNPD_PM_LIST_DB_FILE, 0); 
...
void notifyCosRulesUpdate()
{
	...
	msgBuff.type = CMSG_UPNP_UPDATE_RULES;
	strcpy(msgBuff.content, g_vars.extInterfaceName);

	if (msg_connCliAndSend(CMSG_ID_COS, &msgFd, &msgBuff))
		trace(1, "Error sending CMSG_UPNP_UPDATE_RULES to cos");
}
void enableSpecificRule(char enable, unsigned int extPort, const char *proto)
{
	struct portMap *entry;
	ithread_mutex_lock(&DevMutex);
	char port[16]= {0};

	sprintf(port, "%d", extPort);
	entry = pmlist_FindSpecific(port, proto);
	if (NULL == entry || entry->m_PortMappingEnabled == enable)
	{
		ithread_mutex_unlock(&DevMutex);
		return;
	}
	if (enable)
	{
#ifndef PATCH_MUTI_FORWARD_UPNP_LIST/*copy from pmAddToKernel func*/
		pmlist_AddPortMapping(1,
							  entry->m_PortMappingProtocol,
							  entry->m_ExternalPort,
		     				  entry->m_InternalClient,
		     				  entry->m_InternalPort
			     			 );
#else
		pmlist_AddPortMapping_PeroutingList(1,
							  entry->m_PortMappingProtocol,
							  entry->m_ExternalPort,
		     				  entry->m_InternalClient,
		     				  entry->m_InternalPort
		...
		if (NULL != entry->next)
		{
			if (pmlist_MutiUser_IptablesForwardList(entry->next, 1, entry->m_InternalPort, entry->m_PortMappingProtocol, entry->m_InternalClient) == 0)
			{
				pmlist_AddPortMapping_ForwardList(1,
							  entry->m_PortMappingProtocol,
							  entry->m_ExternalPort,
		     				  entry->m_InternalClient,
		     				  entry->m_InternalPort
	...
	{
#ifndef PATCH_MUTI_FORWARD_UPNP_LIST/*copy from pmDelFromKernel func*/
		pmlist_DeletePortMapping(1,
								 entry->m_PortMappingProtocol,
								 entry->m_ExternalPort,
			     				 entry->m_InternalClient,
			     				 entry->m_InternalPort
		...
		pmlist_DeletePortMapping_PeroutingList(1,
								 entry->m_PortMappingProtocol,
								 entry->m_ExternalPort,
			     				 entry->m_InternalClient,
			     				 entry->m_InternalPort
		...
		pmlist_DeletePortMapping_ForwardList(1,
								 entry->m_PortMappingProtocol,
								 entry->m_ExternalPort,
			     				 entry->m_InternalClient,
			     				 entry->m_InternalPort
	...
	entry->m_PortMappingEnabled = enable;
	updateRulesToFile();
		
	/*added by xianjiantao,17-06-23*/
	notifyCosRulesUpdate();
	/*added end xianjiantao*/
