...
#if CLIENT > 2
#define USBIP_HEARTBEAT_ECHO	0x0010 // added by tf for heartbeat echo cmd , 110530
#define USBIP_HEARTBEAT_REPLY   0x0011 // added by tf for heartbeat reply cmd , 110530
#endif
...
struct usbip_task {
	...
	/* using mutex() instead of completion */
};
...
struct usbip_device{
	...
	/* add a thread to send heartbeat pkts instead of "add_timer(...), by tf, 110811" */
#if CLIENT > 2
	struct usbip_task heartbeat_tx;
#endif
...
/* 
 * make sure each MACRO should inlcude USBIP_EH_SHUTDOWN, because we need wait to finish every kernel thread in stub_shutdown_connection.
 * due to bug[14651], bug[14671].
 * added by tf, 111109
 */
