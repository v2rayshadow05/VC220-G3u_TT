...
/* Modify by chz for 2.6.36 kernel, 2013-05-29 */
...
static int is_clear_halt_cmd(struct urb *urb)
{
	...
	req = (struct usb_ctrlrequest *) urb->setup_packet;
	 ...
	 return (req->bRequest == USB_REQ_CLEAR_FEATURE) &&
		 ...
		 (le16_to_cpu(req->wValue) == USB_ENDPOINT_HALT);	/* modified by tf, le16_to_cpu is needed! */
}
...
static int tweak_clear_halt_cmd(struct urb *urb)
{
	...
	if (ret < 0)	/* using printk instead of uinfo */
...
#if 0	/* hided by tf, 110527; other is no needed! */
