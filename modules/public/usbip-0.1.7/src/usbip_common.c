...
#include <linux/smp_lock.h>	/* added by tf, 110525, for "lock_kernel()" */
#else
#include <linux/mutex.h>
static DEFINE_MUTEX(usbip_mutex);
#endif
...
void usbip_start_threads(struct usbip_device *ud)
{
	 ...
	 */
	int ret;	/* added by tf, 110622 */

	/* before each thread runs, we init_completion(), tf, 111110 */
	init_completion(&ud->tcp_rx.thread_done);
	ret = kernel_thread((int(*)(void *))usbip_thread, (void *)&ud->tcp_rx, 0);
	if (ret < 0) {
	...
	ret = kernel_thread((int(*)(void *))usbip_thread, (void *)&ud->tcp_tx, 0);
	if (ret < 0) {
		printk(KERN_ERR "Creating tcp_tx thread for ud %p failed.\n", ud);
		return;
	}

	/* create thread "heartbeat_tx" by tf, 110811 */
#if CLIENT > 2
	/* before each thread runs, we init_completion(), tf, 111110 */
	init_completion(&ud->heartbeat_tx.thread_done);
	ret = kernel_thread((int(*)(void *))usbip_thread, (void *)&ud->heartbeat_tx, 0);
	if (ret < 0) {
		printk(KERN_ERR "Creating heartbeat_tx thread for ud %p failed.\n", ud);
		return;
	}
#endif
...
#if CLIENT > 2
	wait_for_completion(&ud->heartbeat_tx.thread_done);
#endif
}
...
void usbip_stop_threads(struct usbip_device *ud)
{
	...
	if (ud->tcp_rx.thread != NULL) {
		udbg("sending KILL to rx_thread, thread_done.done = %d\n", ud->tcp_rx.thread_done.done);
		send_sig(SIGKILL, ud->tcp_rx.thread, 1);

		wait_for_completion(&ud->tcp_rx.thread_done);
		udbg("rx_thread for ud %p has finished, thread_done.done = %d\n",ud, ud->tcp_rx.thread_done.done);
	}
...
#if CLIENT > 2
	if (ud->heartbeat_tx.thread != NULL) {
		send_sig(SIGKILL, ud->heartbeat_tx.thread, 1);
		wait_for_completion(&ud->heartbeat_tx.thread_done);
	}	
#endif
}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 36) /* Delete by chz for 2.6.36 kernel, 2013-05-29 */
