...
{                              
	...
	/*
	DBGPRINT(RT_DEBUG_ERROR, "%s dump packet with len = %d\n", __FUNCTION__, len);
	for(j=0;j<len; j++)
	{
		if ( (j%16==0) && (j!=0))
			DBGPRINT(RT_DEBUG_OFF, "\n");
		DBGPRINT(RT_DEBUG_OFF, "%02x ", buf[j]& 0xff);
	}
	*/
	rec = (priv_rec*)buf;
	...
	{
		u8	icmd = *(pos + 5);
		DBGPRINT(RT_DEBUG_TRACE, "Receive Internal Command:%d\n", icmd);
		switch(icmd)
		{
			case DOT1X_DISCONNECT_ENTRY:
			{
				...
				for(i = 0; i<256;i++)
				{
					...
					while (t != NULL)
					{
						DBGPRINT(RT_DEBUG_TRACE, "STA_HASH[%d]="MACSTR"\n", i, MAC2STR(t->addr));
						t = t->hnext;
					}	
				}
#endif				
				...
				{
#ifdef ACCOUNTING_SUPPORT
					memcpy((unsigned char *)&data, pos + 9, sizeof(DOT1X_QUERY_STA_DATA));
					DBGPRINT( RT_DEBUG_TRACE,"handle_read::rx_byte:%lu  tx_byte:%lu rx_pkt:%lu tx_pkt:%lu\n",
									data.rx_bytes, data.tx_bytes, data.rx_packets, data.tx_packets);
					s->final_rx_bytes = data.rx_bytes;
					s->final_tx_bytes = data.tx_bytes;
					s->final_rx_packets = data.rx_packets;
					s->final_tx_packets = data.tx_packets;
#endif
				...
				{
					DBGPRINT(RT_DEBUG_TRACE, "This station(%02x:%02x:%02x:%02x:%02x:%02x) doesn't exist.\n", MAC2STR(sa));
				}
			}
			break;
			...
			break;
#ifdef ACCOUNTING_SUPPORT
			case DOT1X_LOCAL_AUTH_ENTRY:
				{
					struct sta_info *sta;
					DBGPRINT(RT_DEBUG_TRACE, "DOT1X_LOCAL_AUTH_ENTRY\n", sock);
					sta = Ap_get_sta(rtapd, sa, &apidx, ethertype, sock, 1);
					if (!sta)
					{
						return;
					}
					if(rtapd->conf->acct_enable[sta->ApIdx])
					{
						os_get_reltime(&sta->connected_time);
						accounting_sta_start(rtapd, sta);
						DBGPRINT(RT_DEBUG_ERROR, "new station added from internal command for accounting\n");
					}
				}
				break;
#endif
...
int Apd_init_sockets(rtapd *rtapd)
{
...
#ifndef BSS_SKIP_SUPPORT
	for (i = 0; i < rtapd->conf->num_preauth_if; i++)
#else
	for (i = 0; i < rtapd->conf->SsidNum; i++)
	{
	if(rtapd->conf->SsidEnabled[i] && rtapd->conf->preauth_if_enabled[i])
#endif
	{
	...
	for (i = 0; i < rtapd->conf->SsidNum; i++)
	{
	if(rtapd->conf->SsidEnabled[i] && rtapd->conf->eap_if_enabled[i])
#endif
	{
...
#ifdef ACCOUNTING_SUPPORT
	if(rtapd->conf->sum_acct_enable)
	{
	if (accounting_init(rtapd)) {
		DBGPRINT(RT_DEBUG_ERROR, "Accounting initialization failed.");
		return -1;
	}
	}
	else
		DBGPRINT(RT_DEBUG_ERROR, "Accounting is not enabled on any interface");
#endif
