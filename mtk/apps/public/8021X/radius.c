...
void radius_msg_finish_acct(struct radius_msg *msg, unsigned char *secret,
			    size_t secret_len)
{
	const u8 *addr[2];
	size_t len[2];
	...
	if (msg->buf_used > 0xffff) {
		DBGPRINT(RT_DEBUG_ERROR, "RADIUS: Too long messages (%lu)",
...
#ifdef ACCOUNTING_SUPPORT
int Radius_msg_verify(struct radius_msg *msg, u8 *secret, size_t secret_len, struct radius_msg *sent_msg, int authn)
#else
