...
static int add_common_radius_sta_attr(struct apd_data *rtapd,
				      struct sta_info *sta,
...
/* vikas: todo fix
	...
	if (!apd_config_get_radius_attr(req_attr,
	...
	*/
	...
	if (sta->acct_session_id_hi || sta->acct_session_id_lo) {
		...
		if (!Radius_msg_add_attr(msg, RADIUS_ATTR_ACCT_SESSION_ID,
					 (u8 *) buf, os_strlen(buf))) {
			DBGPRINT(RT_DEBUG_ERROR, "Could not add Acct-Session-Id");
			return -1;
		}
	}

#if 0	//def CONFIG_IEEE80211R
	if (rtapd->conf->wpa && wpa_key_mgmt_ft(hapd->conf->wpa_key_mgmt) &&
	    sta->wpa_sm &&
...
int add_common_radius_attr(struct apd_data *rtapd,
			   struct sta_info *sta,
			   struct radius_msg *msg,
			   int ApIdx)
{
	char buf[128];
	struct apd_radius_attr *attr;

	if (!Radius_msg_add_attr(msg, RADIUS_ATTR_NAS_IP_ADDRESS,
				 (u8 *) &rtapd->conf->own_ip_addr, 4)) {
	...
	if (!Radius_msg_add_attr(msg, RADIUS_ATTR_NAS_IDENTIFIER,
				 (u8 *) rtapd->conf->nasId[ApIdx],
				 os_strlen(rtapd->conf->nasId[ApIdx]))) {
		DBGPRINT(RT_DEBUG_ERROR, "Could not add NAS-Identifier");
		return -1;
	}

	os_snprintf(buf, sizeof(buf), RADIUS_802_1X_ADDR_FORMAT ":%s",
		    MAC2STR(rtapd->own_addr[ApIdx]),
		    wpa_ssid_txt(rtapd->conf->Ssid[ApIdx],
				 rtapd->conf->SsidLen[ApIdx]));
	buf[sizeof(buf) - 1] = '\0';
	...
	if (sta && add_common_radius_sta_attr(rtapd, sta, msg) < 0)
		return -1;
	...
	for (attr = req_attr; attr; attr = attr->next) {
		if (!Radius_msg_add_attr(msg, attr->type,
					 attr->val->buf,
					 attr->val->used)) {
			DBGPRINT(RT_DEBUG_ERROR, "Could not add RADIUS "
				   "attribute");
			return -1;
		}
	}*/
...
#ifdef ACCOUNTING_SUPPORT
int os_get_reltime(struct os_time *t)
{
	int res;
	struct timeval tv;
...
#ifdef ACCOUNTING_SUPPORT
	if (authorized && rtapd->conf->acct_enable[sta->ApIdx]) 
	{
...
#ifdef ACCOUNTING_SUPPORT	
	sta = Ap_get_sta(rtapd, sa, apidx, ethertype, SockNum, 0);
#else
			...
			u8 *shared_secret, size_t shared_secret_len, void *data)
{
	...
	switch (msg->hdr->code)
	{
		case RADIUS_CODE_ACCESS_ACCEPT:
			...
			else if (session_timeout_set && (rtapd->conf->session_timeout_set[sta->ApIdx] == 1))   // 1 1 
			{
				Ap_sta_session_timeout(rtapd, sta, ((session_timeout<rtapd->conf->session_timeout_interval[sta->ApIdx]) ? session_timeout : rtapd->conf->session_timeout_interval[sta->ApIdx]));
			}
			else if (session_timeout_set )   // 1 0 
			{
				Ap_sta_session_timeout(rtapd, sta, session_timeout);
			}
			else if (rtapd->conf->session_timeout_set[sta->ApIdx] == 1)   // 0 1
			{
				Ap_sta_session_timeout(rtapd, sta, rtapd->conf->session_timeout_interval[sta->ApIdx]);
			}
			else  // 0 0
				free_flag = 1;
