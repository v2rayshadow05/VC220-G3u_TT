...
#ifdef ACCOUNTING_SUPPORT
struct sta_info* Ap_get_sta(rtapd *apd, u8 *sa, u8 *apidx, u16 ethertype, int sock, int localauth)
#else
...
#ifdef ACCOUNTING_SUPPORT
		s->acct_interim_interval = apd->conf->acct_interim_interval[s->ApIdx];
		accounting_sta_get_id(apd, s);
#endif		
		s->SockNum = sock;
...
void Ap_free_sta(rtapd *apd, struct sta_info *sta)
{
	...
	Ap_sta_no_session_timeout(apd, sta);
#ifdef ACCOUNTING_SUPPORT
	accounting_sta_stop(apd, sta);
#endif
...
//	Ap_free_sta(apd, sta);
	...
	if (!strcmp(apd->prefix_wlan_name, "rai")) {
		accounting_sta_stop(apd, sta);
		accounting_sta_get_id(apd, sta);
		sleep(1);
		os_get_reltime(&sta->connected_time);
		accounting_sta_start(apd, sta);
	}
	else {
		//2.4g TODO
	}
