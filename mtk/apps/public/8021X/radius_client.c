...
static int
Radius_change_server(struct apd_data *rtapd, struct hostapd_radius_server *nserv,
			 struct hostapd_radius_server *oserv, int sock, int auth);
#ifdef ACCOUNTING_SUPPORT		 
#if MULTIPLE_RADIUS
static int Radius_client_init_acct(rtapd *rtapd, int apidx);
#else
static int Radius_client_init_acct(rtapd *rtapd);
#endif
#endif
							 ...
							 u8 *shared_secret, size_t shared_secret_len, void *data), void *data)
{
	...
	{
		handlers = &apd->radius->acct_handlers;
		num = &apd->radius->num_acct_handlers;
	}
	else
#endif
	{
...
static int Radius_client_retransmit(struct apd_data *rtapd, struct radius_msg_list *entry, time_t now)
{
	int s;
#ifdef ACCOUNTING_SUPPORT
	...
	{
#if MULTIPLE_RADIUS
		s = rtapd->radius->mbss_acct_serv_sock[entry->ApIdx];
		serv.sin_addr.s_addr = rtapd->conf->mbss_acct_server[entry->ApIdx]->addr.s_addr;
		serv.sin_port = htons(rtapd->conf->mbss_acct_server[entry->ApIdx]->port);
#else
		s = rtapd->radius->acct_serv_sock;
		serv.sin_addr.s_addr = rtapd->conf->acct_server->addr.s_addr;
		serv.sin_port = htons(rtapd->conf->acct_server->port);
#endif
	}
	else
	...
	entry->attempts++;
#ifdef ACCOUNTING_SUPPORT
	if((sendto( s, entry->msg->buf, entry->msg->buf_used, 0, ( struct sockaddr * )&serv, sizeof(serv) ))< 0)
#else
...
static void Radius_client_timer(void *eloop_ctx, void *timeout_ctx)
{
...
#ifdef ACCOUNTING_SUPPORT
	int mbss_acct_failover[MAX_MBSSID_NUM];
#endif
#else
	int auth_failover = 0;
#ifdef ACCOUNTING_SUPPORT
	...
	while (entry)
	{
		...
		if (entry->attempts > RADIUS_CLIENT_NUM_FAILOVER)
		{
			...
			{
#if MULTIPLE_RADIUS
				mbss_acct_failover[entry->ApIdx]++;
#else
				acct_failover++;
#endif	
			}
#endif
			DBGPRINT(RT_DEBUG_WARN, "Radius_client_timer : MsgType:%d : Failed retry attempts(%d) \n", entry->msg_type, RADIUS_CLIENT_NUM_FAILOVER);
		}
	...
	{	
		if (mbss_auth_failover[i] && rtapd->conf->mbss_num_auth_servers[i] > 1)
		{
			...
			DBGPRINT(RT_DEBUG_WARN, "Radius_client_timer : ready to change AUTH RADIUS server for %s%d\n", rtapd->prefix_wlan_name, i);
		}
#ifdef ACCOUNTING_SUPPORT
		if (mbss_acct_failover[i] && rtapd->conf->mbss_num_acct_servers[i] > 1)
		{
			struct hostapd_radius_server *next, *old;
			old = rtapd->conf->mbss_acct_server[i];

			next = old + 1;
			if (next > &(rtapd->conf->mbss_acct_servers[i][rtapd->conf->mbss_num_acct_servers[i]- 1]))
				next = rtapd->conf->mbss_acct_servers[i];
			rtapd->conf->mbss_acct_server[i] = next;
			Radius_change_server(rtapd, next, old, rtapd->radius->mbss_acct_serv_sock[i], 1);
			DBGPRINT(RT_DEBUG_WARN, "Radius_client_timer : ready to change ACCT RADIUS server for %s%d\n", rtapd->prefix_wlan_name, i);
		}
#endif
	}
#else
	if (auth_failover && rtapd->conf->num_auth_servers > 1)
	{
		...
		DBGPRINT(RT_DEBUG_WARN, "==> Radius_client_timer : ready to change AUTH RADIUS server \n");	
	}
#ifdef ACCOUNTING_SUPPORT
	if (acct_failover && rtapd->conf->num_acct_servers > 1)
	{
		struct hostapd_radius_server *next, *old;
		old = rtapd->conf->acct_server;

		next = old + 1;
		if (next > &(rtapd->conf->acct_servers[rtapd->conf->num_acct_servers - 1]))
			next = rtapd->conf->acct_servers;
		rtapd->conf->acct_server = next;
		Radius_change_server(rtapd, next, old, rtapd->radius->acct_serv_sock, 1);
		DBGPRINT(RT_DEBUG_WARN, "==> Radius_client_timer : ready to change ACCT RADIUS server \n");	
	}
#endif
#endif			
}

static void Radius_client_list_add(struct apd_data *rtapd, struct radius_msg *msg,
...
#ifdef ACCOUNTING_SUPPORT
static void Radius_client_list_del(struct apd_data *rtapd,
				   RadiusType msg_type, u8 ApIdx)
{
	struct radius_msg_list *entry, *prev, *tmp;
	...
	while (entry) 
	{
		if ((entry->msg_type == msg_type) && (entry->ApIdx == ApIdx))
		{
			if (prev)
				prev->next = entry->next;
			else
...
int Radius_client_send(struct apd_data *rtapd, struct radius_msg *msg, RadiusType msg_type, u8 ApIdx)
{
	...
	if (msg_type == RADIUS_ACCT_INTERIM) 
	{
		...
		Radius_client_list_del(rtapd, msg_type, ApIdx);
	}

	if (msg_type == RADIUS_ACCT || msg_type == RADIUS_ACCT_INTERIM) 
	{
#if MULTIPLE_RADIUS
		if ((rtapd->conf->mbss_acct_server[ApIdx] && rtapd->radius->mbss_acct_serv_sock[ApIdx]) < 0)
			Radius_client_init_acct(rtapd->radius, ApIdx);

		shared_secret = rtapd->conf->mbss_acct_server[ApIdx]->shared_secret;
		shared_secret_len = rtapd->conf->mbss_acct_server[ApIdx]->shared_secret_len;
		//vikas: todo
		radius_msg_finish_acct(msg, shared_secret, shared_secret_len);
		name = "accounting";
		s = rtapd->radius->mbss_acct_serv_sock[ApIdx];
		rtapd->conf->mbss_acct_server[ApIdx]->requests++;
		serv.sin_addr.s_addr = rtapd->conf->mbss_acct_server[ApIdx]->addr.s_addr;
		serv.sin_port = htons(rtapd->conf->mbss_acct_server[ApIdx]->port);		
#else
		if (rtapd->conf->acct_server && rtapd->radius->acct_serv_sock < 0)
			Radius_client_init_acct(rtapd);
		
		shared_secret = rtapd->conf->acct_server->shared_secret;
		shared_secret_len = rtapd->conf->acct_server->shared_secret_len;
		radius_msg_finish_acct(msg, shared_secret, shared_secret_len);
		name = "accounting";
		s = rtapd->radius->acct_serv_sock;
		rtapd->conf->acct_server->requests++;
		serv.sin_addr.s_addr = rtapd->conf->acct_server->addr.s_addr;
		serv.sin_port = htons(rtapd->conf->acct_server->port);
#endif
	} 
...
#ifdef ACCOUNTING_SUPPORT
	if(msg_type == RADIUS_ACCT_INTERIM)		//sent msg stored by sock_type ie acct not interim
...
void Radius_client_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	if (msg_type == RADIUS_ACCT || msg_type == RADIUS_ACCT_INTERIM) 
	{
		DBGPRINT(RT_DEBUG_TRACE,"%s: Setting Acct handlers\n", __FUNCTION__);
		handlers = rtapd->radius->acct_handlers;
		num_handlers = rtapd->radius->num_acct_handlers;
		//rconf = conf->acct_server;	vikas: todo fix
	} else 
#endif
	{
		handlers = rtapd->radius->auth_handlers;
		num_handlers = rtapd->radius->num_auth_handlers;
		//rconf = conf->auth_server;	vikas: todo fix
	}
	...
	{
#if 0
		DBGPRINT(RT_DEBUG_TRACE,"%s: no req found corresponding to this receive msg::: type:%d, identifier:%d\n",
			__FUNCTION__,msg_type,msg->hdr->identifier);
		
		fail_req = rtapd->radius->msgs;
		while (fail_req)
		{
			/* TODO: also match by src addr:port of the packet when using
			 * alternative RADIUS servers (?) */
			DBGPRINT(RT_DEBUG_TRACE,"%s: req msg::: type:%d, identifier:%d\n",
								__FUNCTION__,fail_req->msg_type,fail_req->msg->hdr->identifier); 
			fail_req = fail_req->next;
		}
#endif
		goto fail;
	}
...
#if 0
	DBGPRINT(RT_DEBUG_TRACE,"%s: Req found corresponding to this receive msg::: type:%d, identifier:%d\n",
			__FUNCTION__,msg_type,msg->hdr->identifier);
#endif	
...
#ifdef ACCOUNTING_SUPPORT
static void radius_client_update_acct_msgs(struct apd_data *rtapd,
					   ...
					   size_t shared_secret_len)
{
	...
	for (entry = rtapd->radius->msgs; entry; entry = entry->next) {
...
/*Comment by rory
	...
	DBGPRINT(RT_DEBUG_TRACE,"%s bind sock:%d to Serv.Sin_port %d\n", __FUNCTION__, sock,  serv.sin_port);
#endif
...
#ifdef ACCOUNTING_SUPPORT
		if (rtapd->radius->mbss_acct_serv_sock[i] >= 0 && rtapd->conf->mbss_acct_servers[i] &&
			rtapd->conf->mbss_acct_server[i] != rtapd->conf->mbss_acct_servers[i])
		{
			oserv = rtapd->conf->mbss_acct_server[i];
			rtapd->conf->mbss_acct_server[i] = rtapd->conf->mbss_acct_servers[i];
			Radius_change_server(rtapd, rtapd->conf->mbss_acct_server[i], oserv, rtapd->radius->mbss_acct_serv_sock[i], 0);
		}
#endif
	}
#else
...
#ifdef ACCOUNTING_SUPPORT
	if (rtapd->radius->acct_serv_sock >= 0 && rtapd->conf->acct_servers &&
		rtapd->conf->acct_server != rtapd->conf->acct_servers)
	{
		oserv = rtapd->conf->acct_server;
		rtapd->conf->acct_server = rtapd->conf->acct_servers;
		Radius_change_server(rtapd, rtapd->conf->acct_server, oserv, rtapd->radius->acct_serv_sock, 0);
	}
#endif
#endif
...
#if MULTIPLE_RADIUS
static int Radius_client_init_auth(struct apd_data *rtapd, int apidx)
{		
...
#else
static int Radius_client_init_auth(struct apd_data *rtapd)
{
...
#ifdef ACCOUNTING_SUPPORT
#if MULTIPLE_RADIUS
int Radius_client_init_acct(struct apd_data *rtapd, int apidx)
{		
	if (rtapd->conf->mbss_acct_server[apidx]->addr.s_addr == 0)
	{
		DBGPRINT(RT_DEBUG_ERROR, "Radius_client_init_acct: can't create acct RADIUS socket for %s%d (it's invalid IP)\n", rtapd->prefix_wlan_name, apidx);
		return -1;
	}

	rtapd->radius->mbss_acct_serv_sock[apidx] = socket(PF_INET, SOCK_DGRAM, 0);
	if (rtapd->radius->mbss_acct_serv_sock[apidx] < 0)
	{
		perror("socket[PF_INET,SOCK_DGRAM]");
		return -1;
	}
	else
		DBGPRINT(RT_DEBUG_TRACE, "Radius_client_init_acct: Created acct RADIUS socket:%d for ApIdx%d\n", apidx, rtapd->radius->mbss_acct_serv_sock[apidx]);

	if (Radius_change_server(rtapd, rtapd->conf->mbss_acct_server[apidx], NULL, rtapd->radius->mbss_acct_serv_sock[apidx], 0))
		return -1;

	if (eloop_register_read_sock(rtapd->radius->mbss_acct_serv_sock[apidx], Radius_client_receive, rtapd, (void *) RADIUS_ACCT))
	{
		DBGPRINT(RT_DEBUG_ERROR,"Could not register read socket for accounting server - iface-%d\n", apidx);
		return -1;
	}
	else
		DBGPRINT(RT_DEBUG_TRACE,"register read socket:%d for accounting server - iface-%d\n", apidx, rtapd->radius->mbss_acct_serv_sock[apidx]);
	return 0;
}
#else
int Radius_client_init_acct(struct apd_data *rtapd)
{
	rtapd->radius->acct_serv_sock = socket(PF_INET, SOCK_DGRAM, 0);
	if (rtapd->radius->acct_serv_sock < 0)
	{
		perror("socket[PF_INET,SOCK_DGRAM]");
		return -1;
	}

	Radius_change_server(rtapd, rtapd->conf->acct_server, NULL, rtapd->radius->acct_serv_sock, 0);

	if (eloop_register_read_sock(rtapd->radius->acct_serv_sock, Radius_client_receive, rtapd, (void *) RADIUS_ACCT))
	{
		DBGPRINT(RT_DEBUG_ERROR,"Could not register read socket for accounting server\n");
		return -1;
	}
	return 0;
}
...
int Radius_client_init(struct apd_data *rtapd)
{
...
#ifdef ACCOUNTING_SUPPORT
			rtapd->radius->mbss_acct_serv_sock[i] = -1;
#endif
		}
		bReInit= 0;
#else
...
#ifdef ACCOUNTING_SUPPORT
		// Create socket for acct RADIUS
		if ((rtapd->radius->mbss_acct_serv_sock[i] < 0)
#ifdef BSS_SKIP_SUPPORT
				&&(rtapd->conf->SsidEnabled[i] && rtapd->conf->acct_enable[i])
#endif
			)
		{
			if (rtapd->conf->mbss_acct_server[i] && (!Radius_client_init_acct(rtapd, i)))
					ready_acct_sock_count++;		
		}
		else
			ready_acct_sock_count++;
#endif
...
#ifdef ACCOUNTING_SUPPORT
	if (ready_acct_sock_count == 0)
	{
		DBGPRINT(RT_DEBUG_ERROR, "Radius_client_init : no any acct RADIUS socket ready \n");
		return -1;
	}
	else
		DBGPRINT(RT_DEBUG_TRACE, "Radius_client_init : ready_acct_sock_count %d \n", ready_acct_sock_count);
#endif

	if (
		(rtapd->conf->radius_retry_primary_interval && !bReInit && ready_sock_count > 0)
#ifdef ACCOUNTING_SUPPORT
		&& (ready_acct_sock_count > 0)
#endif
		)
		eloop_register_timeout(rtapd->conf->radius_retry_primary_interval, 0, Radius_retry_primary_timer, rtapd, NULL);
	...
	if( rtapd->radius->acct_serv_sock < 0)
	{
		if (rtapd->conf->acct_server && Radius_client_init_acct(rtapd))
			return -1;
	}
#endif
...
#ifdef ACCOUNTING_SUPPORT
	free(rtapd->radius->acct_handlers);
#endif
	free(rtapd->radius);
	rtapd->radius = NULL;
}
