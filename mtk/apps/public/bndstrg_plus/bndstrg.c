...
#define TP_DBG(x, ...) //printf("###%s(%d) "x"\n", __FUNCTION__, __LINE__, ##__VA_ARGS__)

#define BNDSTRG_DAEMON_VER	"3.1.7(E-Link v0.2)"
#define BNDSTRG_DAT_FILE_PATH	"/etc/bndstrg.conf"
/* optimize the table size limit caused deamon exit */
#ifndef BNDSTRG_TBLSIZ_OPT
#define BNDSTRG_TBLSIZ_OPT
extern struct bndstrg bndstrg;
#endif
#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
int	PriorityList[] = {  fBND_STRG_PRIORITY_5G_RSSI,
...
char *entry_match_steering_str[]={
		...
		"LB_RSSI",
		"LB_MCS",
#ifdef  LIMIT_5G_NUM_SUPPORT
		"LIMIT_5G_NUM_PRE",//add by yp
#endif
		"DEFAULT_2G",
		"DEFAULT_5G",
		"RSSI_DOWNSTEER",
...
#ifndef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2
static u32 find_max_elapsed_time_entry(struct bndstrg_cli_table *table);
#endif
#endif
...
#ifdef  LIMIT_5G_NUM_SUPPORT
int bndstrg_get_maxstanum_perssid(struct bndstrg *bndstrg,int Band);
#endif
        ...
        if(inf->bValid)
        {
            ...
            if(!strcmp(inf->ucIfName, inf_status_rsp->ucIfName))
            {
                ...
                if(ctrl_iface->Active5GCount > 0)
                {
					...
					#ifdef LIMIT_5G_NUM_SUPPORT
				//	if(!table->AC_MbssMaxNumLimit)  //add by yp
					{
						table->AC_MbssMaxNumLimit = bndstrg_get_maxstanum_perssid(table->bndstrg,BAND_5G_H|BAND_5G_L);
                                        #ifdef VENDOR_IOS_SUPPORT
						table->IOSNumLimit = table->AC_MbssMaxNumLimit;//add by yp;
                                        #endif						
					}
					#endif
                }
...
if(table->BndStrgMode == POST_CONNECTION_STEERING && entry->connected_band != BAND_INVALID){
...
#ifndef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2
	u32 entry_index = 0;
	struct bndstrg_cli_entry *del_entry = NULL; 
	struct bndstrg_iface *inf = NULL;
#else
...
#ifdef BNDSTRG_TBLSIZ_OPT
#ifndef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2
		entry_index = find_max_elapsed_time_entry(table);
		if(entry_index < BND_STRG_MAX_TABLE_SIZE) {
			del_entry = &table->Entry[entry_index];
			inf = bndstrg_get_interface_by_channel(&bndstrg.ctrl_iface,del_entry->Channel);
			if (inf) {
				bndstrg_accessible_cli(&bndstrg,inf,del_entry,CLI_DEL);			
			}
		
			bndstrg_delete_entry(table,del_entry->Addr,del_entry->TableIndex);
		} 
		else
#else
		...
		struct bndstrg_msg *msg)
{
	...
	struct bndstrg_iface *inf = NULL;
	struct bndstrg_cli_table *table = &bndstrg->table;
	u8 			channel = cli_event->Channel, new_band = BAND_INVALID;
	...
	switch (entry->state) {
	case ENTRY_INIT:
		if((FrameType == APMT2_PEER_PROBE_REQ) && table->fFastForwardAlgo && !IS_BND_STRG_DUAL_BAND_CLIENT(entry->Control_Flags)){
			/* Make decision in FastForwardAlgo on probe*/
			if(IS_2G_BAND(inf->Band)) {
				if ((entry->statistics[IDX_2G].ProbeReqCount > 1) && entry->statistics[IDX_2G].Rssi < table->RssiWorst){
					entry->Control_Flags |= fBND_STRG_CLIENT_ALLOW_TO_CONNET_2G;
					new_band = BAND_2G;
				}
			} else if(entry->statistics[IDX_5GL].Rssi > table->RssiLow){
				entry->Control_Flags |= fBND_STRG_CLIENT_ALLOW_TO_CONNET_L5G;
				new_band = BAND_5G_L;
			}
			if(new_band != BAND_INVALID && new_band != entry->band){
				entry->match_steered_rule_id[IDX_2G] = fBND_STRG_PRIORITY_FAST_FORWARD;
				entry->match_steered_rule_id[IDX_5GL] = fBND_STRG_PRIORITY_FAST_FORWARD;
				entry->state = ENTRY_READY_TO_ASSOC;
				bndstrg_client_band_update(bndstrg, entry, new_band);
				BND_STRG_PRINTQAMSG(table, entry, ("%02x:%02x:%02x:%02x:%02x:%02x FFAlgo allow to connect [band:%s] \n"),
					PRINT_MAC(entry->Addr), bndstrg_get_entry_band(new_band));
			}
		}

		else if((FrameType == APMT2_PEER_AUTH_REQ) && table->fFastForwardAlgo && !IS_BND_STRG_DUAL_BAND_CLIENT(entry->Control_Flags)) {
			if(IS_2G_BAND(inf->Band)) {
				if((entry->band == BAND_INVALID) && (entry->statistics[IDX_2G].Rssi < table->RssiWorst)){
					new_band = BAND_2G;
					entry->Control_Flags |= fBND_STRG_CLIENT_ALLOW_TO_CONNET_2G;
				}
			} else if((entry->band == BAND_INVALID) && (entry->statistics[IDX_5GL].Rssi > table->RssiLow)){
				new_band = BAND_5G_L;
				entry->Control_Flags |= fBND_STRG_CLIENT_ALLOW_TO_CONNET_L5G;
			}
			if(new_band != BAND_INVALID && new_band != entry->band){
	...
	case ENTRY_READY_TO_ASSOC:
...
#ifndef TCSUPPORT_WLAN_BNDSTRG_CUSTOM1
		...
		{
			printf("%s() - Get DISASSOC_REQ from [%02x:%02x:%02x:%02x:%02x:%02x] when ENTRY_READY_TO_ASSOC.\n\n",
		...
		if(FrameType == APMT2_PEER_AUTH_REQ) 
		{
			...
			if (entry->biOSEntry)
			{
				BND_STRG_PRINTQAMSG(table, entry, (" %02x:%02x:%02x:%02x:%02x:%02x [band:%s][Channel:%d] re-steering needed! \n"),
					PRINT_MAC(entry->Addr),bndstrg_get_entry_band(entry->band), entry->Channel);
				DBGPRINT(DEBUG_OFF,("[re-steering needed] iOS entry, no need to delete driver entry\n"));
				break;
			}
#endif
			if(table->BndStrgMode != POST_CONNECTION_STEERING){
...
#endif /* BND_STRG_QA */
//#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
				break;/* added by huaming, do not handle this case */
//#endif
#ifndef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2
				get_current_system_tick(&entry->tp);
...
#endif /*WPS_SUPPORT*/		
			{
                        	entry->bConnStatus = FALSE;
    		                bndstrg_update_disassoc_info(bndstrg, inf, entry, msg);
#ifdef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2
			entry->FPT_steered_cnt = 0; //FPT 0615, reset FPT steer count once receive the disassoc from station
			//FPT 0615 dbg remove later
			BND_STRG_DBGPRINT(DEBUG_OFF, "Addr=%02x:%02x:%02x:%02x:%02x:%02x, [STAT: ENTRY_ASSOC_RCD]FPT counter Reset due to Disassoc from sta\n", PRINT_MAC(entry->Addr));
#endif
		    	        get_current_system_tick(&entry->tp);
			        entry->state = ENTRY_DELETE_RCD;
		        }
	...
	case ENTRY_OPER_STEER_ACTIVE:
		if(FrameType == APMT2_PEER_ASSOC_REQ) {
		    ...
		    entry->bConnStatus = FALSE;
#ifdef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2
			// FPT 0615, remove later
		   	BND_STRG_DBGPRINT(DEBUG_OFF, "Addr=%02x:%02x:%02x:%02x:%02x:%02x, [STAT: ENTRY_OPER_STEER_ACTIVE] FPT counter should  NOT Reset \n", PRINT_MAC(entry->Addr));
#endif
		    bndstrg_update_disassoc_info(bndstrg, inf, entry, msg);
			get_current_system_tick(&entry->tp);
		}
										 ...
										 u8 dump_cli_status)
{
	...
	if (dump_cli_status) {
		BND_STRG_DBGPRINT(DEBUG_OFF,
						  	"Rssi = %4d, FirstAuthTime %s AuthReqCount = %3u FirstProbeTime %s ProbeReqCount = %u\n" 
						  	"\t\t\tFirstAssocTime %s AssocCount = %u FirstClientDelTime %s MacDelCount = %u\n",
							statistics->Rssi, statistics->FirstAuthTime, statistics->AuthReqCount, statistics->FirstProbeTime , statistics->ProbeReqCount,
							statistics->FirstAssocTime, statistics->AssocCount,statistics->FirstClientDelTime, statistics->MacDelCount);
...
#endif
#ifdef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2			
		BND_STRG_DBGPRINT(DEBUG_OFF, "\t\tFPT count:%d\n", entry->FPT_steered_cnt);
#endif			
		if (single_band == 1)
		{
			if(entry->match_steered_rule_id[IDX_2G] == fBND_STRG_PRIORITY_FAST_FORWARD ||
				entry->match_steered_rule_id[IDX_5GL] == fBND_STRG_PRIORITY_FAST_FORWARD ){
				BND_STRG_DBGPRINT(DEBUG_OFF, "\t\tFastForwardAlgo\n");
			} else
	...
	BND_STRG_DBGPRINT(DEBUG_OFF,
		...
		for (i = 0; i < table->PriorityListSize; i++){
			if(table->PriorityList[i] < fBND_STRG_PRIORITY_MAX)
				bndstrg_dbg_printf("\t\t %d: %s\n",table->PriorityList[i], entry_match_steering_str[table->PriorityList[i]]);
			else
				bndstrg_dbg_printf("\t\t\t Invalid PriorityID:%d",table->PriorityList[i]);
		}
		bndstrg_dbg_printf("\n");
		...
		if(table->WhiteEntryListSize){
			...
			for(i=0; i < BND_STRG_MAX_DISABLE_LIST; i++){
				if(IS_VALID_MAC(table->WhiteEntryList[i])){
					bndstrg_dbg_printf("\t\t[%d]:%02x:%02x:%02x:%02x:%02x:%02x \n", i, PRINT_MAC(table->WhiteEntryList[i]));
					count++;
				}
	...
	struct bndstrg_cli_table *table)
{
	...
	for (i = 0; i < BND_STRG_MAX_TABLE_SIZE; i++) {
		...
		if (entry->bValid == TRUE &&
			...
			AuthReqCount == 0) /*delete entry not send auth*/
		{
...
#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
                                 || (table->PriorityList[i] == fBND_STRG_PRIORITY_RSSI_DOWNSTEER) 
		...
		while((token = strtok(NULL, ";")) != NULL) {
			...
			if(token_value > 0){
				if (j == 0)
					...
					table->DetectInterval = token_value;
				else if (j == 2)
					table->IdleThreshold = token_value;
				j++;
		    }
		}
		printf("%s() - dwelltime = %d, DetectInterval = %d, IdleThreshold = %d\n\n", 
			__func__, table->dwell_time, table->DetectInterval, table->IdleThreshold);
#else
		token = strtok(NULL, ";");
		token_value = strtol(token, NULL, 10);
		if (token_value > 0)
...
break;
	...
	case BND_SET_RSSIWORST:
		token = strtok(NULL, ";");
		token_value = strtol(token, NULL, 10);
		table->RssiWorst = token_value;
	break;
...
u8 bndstrg_chanload_heavy_update(struct bndstrg *bndstrg)
{
	...
	for(i=0;i<ctrl_iface->Size;i++){
		...
		if(inf->bValid){
			...
			//printf("%s() - Band %d Heavy Count = %d\n\n", __func__, inf->Band, inf->chanload_heavy_cnt);
		}
	}	
	return TRUE;
}
		...
		if ((entry->bValid == TRUE) && (entry->bConnStatus))
		{
				...
				IS_BND_STRG_H5G_L5G_BAND_CLIENT(entry->Control_Flags))
			{
				switch(entry->band 
					...
					case BAND_2G:
						if (entry->statistics[IDX_2G].got_cli_status && 
#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
							entry->statistics[IDX_2G].data_tx_byte.avg  <= table->IdleThreshold &&
							entry->statistics[IDX_2G].data_rx_byte.avg  <= table->IdleThreshold
#else
	                                         	entry->statistics[IDX_2G].data_tx_byte.avg  <= IDLE_RXTX_BYTE_COUNT &&
							entry->statistics[IDX_2G].data_rx_byte.avg  <= IDLE_RXTX_BYTE_COUNT
#endif 
                                                ){
							entry->IsIdle = TRUE;
						}else
						...
						printf("%s() - 2G Tx: %d, Rx:%d\n", __func__,entry->statistics[IDX_2G].data_tx_byte.avg,
							entry->statistics[IDX_2G].data_rx_byte.avg); 
						break;
					case BAND_5G_L:
						bndstrg_worst_sta_update(bndstrg, entry, BAND_5G_L, IDX_5GL);
						if ( entry->statistics[IDX_5GL].got_cli_status &&
#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
							 entry->statistics[IDX_5GL].data_tx_byte.avg  <= table->IdleThreshold &&
							 entry->statistics[IDX_5GL].data_rx_byte.avg  <= table->IdleThreshold 
#else
                                                         entry->statistics[IDX_5GL].data_tx_byte.avg  <= IDLE_RXTX_BYTE_COUNT &&
							 entry->statistics[IDX_5GL].data_rx_byte.avg  <= IDLE_RXTX_BYTE_COUNT
#endif  
                                                ){
							entry->IsIdle = TRUE;
						}else
						...
						printf("%s() - 5G Tx: %d, Rx:%d\n", __func__,entry->statistics[IDX_5GL].data_tx_byte.avg,
							entry->statistics[IDX_5GL].data_rx_byte.avg);
						break;
					case BAND_5G_H:
				...
				printf("%s() - entry %02x:%02x:%02x:%02x:%02x:%02x %s is %s.\n\n", __func__, PRINT_MAC(entry->Addr),
							entry->band == BAND_2G ? "2G" : "5G",
							entry->IsIdle ? "IDLE" : "BUSY");
			}
		}
		if(count >= table->Size)
		{
...
#ifdef LIMIT_5G_NUM_SUPPORT
int bndstrg_get_connectnum(struct bndstrg *bndstrg, u8 Band)       //add by yp
{
	int Lstanum = 0;
	int Hstanum = 0;
	int nstanum = 0;
	...
	for(i = 0;i<BND_STRG_MAX_TABLE_SIZE;i++)
	{
		struct bndstrg_cli_entry 	*pEntry = &table->Entry[i];
		if(pEntry->bConnStatus)
		{
			if (IS_2G_BAND(pEntry->connected_band)){
				nstanum++;//todo?
			}
			else if(IS_5G_L_BAND(pEntry->connected_band)){
				Lstanum++;
			}
			else if(IS_5G_H_BAND(pEntry->connected_band)){
				Hstanum++;
			}
		}
	}
	...
	{
		stanum = Lstanum+ Hstanum;
	}
	
	return stanum;
}
int bndstrg_get_interface_index(struct bndstrg *bndstrg, u8 Band)
{
    u8 i = 0;
	struct bndstrg_ctrl_iface *ctrl_iface = &bndstrg->ctrl_iface;
    struct bndstrg_iface *inf = NULL;
    for(i=0; i < ctrl_iface->Size; i++)
    {
        ...
        if(inf->bValid)
        {
  			if (inf->bInfReady && (inf->Band & Band))
   				return inf->wifiindex;			
        }
    }
...
int bndstrg_get_maxstanum_perssid(struct bndstrg *bndstrg,int Band)
{
	...
	wifiindex = bndstrg_get_interface_index(bndstrg,Band);
	memset(nodeName,0,sizeof(nodeName));	
	sprintf(nodeName, "WLan11ac_Entry%d", wifiindex);
	memset(buf, 0, sizeof(buf));
	tcapi_get(nodeName, "MbssMaxStaNum", buf);
	mbssmaxnum = atoi(buf);
	
	return 	(mbssmaxnum == 0)?64:mbssmaxnum;
}
	...
	for(i=0; i < table->PriorityListSize; i++)
	{
		if (entry->bConnStatus) {
			if ((table->PriorityList[i] != fBND_STRG_PRIORITY_RSSI_DOWNSTEER) &&
			    ...
			    (table->PriorityList[i] != fBND_STRG_PRIORITY_RSSI_UPSTEER)
#ifdef LIMIT_5G_NUM_SUPPORT
			    && (table->PriorityList[i] != fBND_STRG_PRIORITY_5G_NUM_LIMIT_POST) // add by yp
#endif
#ifdef VENDOR_IOS_SUPPORT
		...
		switch(table->PriorityList[i]){
...
#else
					BND_STRG_PRINTQAMSG(table,entry,"IOS_UPSTEER+++Addr=%02x:%02x:%02x:%02x:%02x:%02x,active_client_5G=%d,IOSNumLimit=%d\n",
						PRINT_MAC(entry->Addr),table->active_client_5G,table->IOSNumLimit);
					if ((table->active_client_5G < table->IOSNumLimit) && 
						(entry->good_rssi_cnt >= RSSI_CHECK_COUNT))
#endif
					{
			...
			case fBND_STRG_PRIORITY_RSSI_DOWNSTEER:
				...
				{
#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
					printf("%s() - Low RSSI: %d, Target Heavy: %d\n\n", __func__, 
						entry->low_rssi_bad_cnt,inf_2nd_band->chanload_heavy_cnt);
#elif TCSUPPORT_WLAN_BNDSTRG_CUSTOM1
					BND_STRG_PRINTQAMSG(table,entry,"RSSI_DOWNSTEER+++Addr=%02x:%02x:%02x:%02x:%02x:%02x,low_rssi_bad_cnt=%d\n",
						PRINT_MAC(entry->Addr),entry->low_rssi_bad_cnt);
#endif
					if (entry->low_rssi_bad_cnt >= 
#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
                                                    table->DetectInterval
#else 
                                                    RSSI_CHECK_COUNT
#endif
                                        ){
			...
			case fBND_STRG_PRIORITY_RSSI_UPSTEER:
				...
				{
#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
					printf("%s() - Good RSSI: %d, Target Heavy: %d\n\n", __func__,
						entry->good_rssi_cnt,inf_1st_band->chanload_heavy_cnt);
#else
                                        BND_STRG_PRINTQAMSG(table,entry,"RSSI_UPSTEER+++Addr=%02x:%02x:%02x:%02x:%02x:%02x,good_rssi_cnt=%d\n",
						PRINT_MAC(entry->Addr),entry->good_rssi_cnt);
#endif
#ifdef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2
					//FPT dbg remove later
					BND_STRG_PRINTQAMSG(table,entry,"low_rssi_bad_cnt=%d, FPT_steered_cnt:%d\n", entry->low_rssi_bad_cnt, entry->FPT_steered_cnt);

					/* reset FPT steered count */
					if (entry->low_rssi_bad_cnt >= RSSI_CHECK_COUNT && entry->FPT_steered_cnt != 0)
					{
						entry->FPT_steered_cnt = 0;
						entry->low_rssi_bad_cnt = 0;
						BND_STRG_DBGPRINT(DEBUG_OFF,"Addr=%02x:%02x:%02x:%02x:%02x:%02x, FPT counter Reset due to RSSI is bad\n",
							PRINT_MAC(entry->Addr));
					}
#endif
					if (entry->good_rssi_cnt >= 
#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
                                                    table->DetectInterval
#else 
                                                    RSSI_CHECK_COUNT
#endif
#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
                                            && inf_1st_band->chanload_heavy_cnt < inf_1st_band->chanload_heavy_max_cnt
#endif
#ifdef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2
                                           && entry->FPT_steered_cnt == 0
#endif
			...
			case fBND_STRG_PRIORITY_LB_CND_MCS: 
				...
				break;
#ifdef LIMIT_5G_NUM_SUPPORT
			case fBND_STRG_PRIORITY_5G_NUM_LIMIT_PRE:  //add by yp				
				ac_connectednum = bndstrg_get_connectnum(bndstrg,BAND_5G_L|BAND_5G_H); 
				//ac_mbssmaxnum = bndstrg_get_maxstanum_perssid(bndstrg,BAND_5G_L|BAND_5G_H);
                ac_mbssmaxnum = table->AC_MbssMaxNumLimit;				
                BND_STRG_PRINTQAMSG(table,entry,"5G_LIMIT_PRE+++Addr=%02x:%02x:%02x:%02x:%02x:%02x,G5_ConnectedNum:%d,G5_ConnectedlimitNum:%d\n\r",
						PRINT_MAC(entry->Addr),ac_connectednum,ac_mbssmaxnum);
				if(ac_connectednum >= ac_mbssmaxnum)
				{
					entry->Control_Flags |= allow_to_connect_band_flag[1];
					done = TRUE;
					band = pre_band[1];
			...
			case fBND_STRG_PRIORITY_5G_NUM_LIMIT_POST: //add by yp
				if((entry->bConnStatus) &&
					((entry->band & pre_band[1]) == pre_band[1]) &&
					(statistics_2nd_band->got_cli_status) && 
					(entry->connected_band == pre_band[1]))
				{
					ac_connectednum = bndstrg_get_connectnum(bndstrg,BAND_5G_L|BAND_5G_H); 
					//ac_mbssmaxnum = bndstrg_get_maxstanum_perssid(bndstrg,BAND_5G_L|BAND_5G_H);
					ac_mbssmaxnum = table->AC_MbssMaxNumLimit;	
					BND_STRG_PRINTQAMSG(table,entry,"5G_LIMIT_POST+++Addr=%02x:%02x:%02x:%02x:%02x:%02x,G5_ConnectedNum:%d,Operation_steered_5G=%d,G5_ConnectedlimitNum:%d\n\r",
						PRINT_MAC(entry->Addr),ac_connectednum,table->Operation_steered_num_5G,ac_mbssmaxnum);
					if(ac_connectednum + table->Operation_steered_num_5G>= ac_mbssmaxnum)
					{
						//entry->Operation_steered = TRUE;
						entry->Control_Flags |= allow_to_connect_band_flag[1];
						band = pre_band[1];
...
#ifdef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2
		entry->BlackListTimer = BLACK_LIST_TIME;   //FPT 0615 modify
#endif
	}
	return TRUE;
}
		...
		struct bndstrg *bndstrg)
{
    ...
    if (!IS_BND_STRG_DUAL_BAND_CLIENT(entry->Control_Flags)) 
	{
        if (entry->Control_Flags & fBND_STRG_CLIENT_IS_2G_ONLY)
		{
			...
			entry->Control_Flags |= fBND_STRG_CLIENT_ALLOW_TO_CONNET_2G;
			band = BAND_2G;
#ifdef TCSUPPORT_WLAN_BNDSTRG_CUSTOM1
			BND_STRG_PRINTQAMSG(table,entry,"++++2G_ONLY++Addr=%02x:%02x:%02x:%02x:%02x:%02x,\n\r",
						PRINT_MAC(entry->Addr));
#endif
		}
		else if(entry->Control_Flags & fBND_STRG_CLIENT_IS_5G_ONLY)
		{
            ...
            else if(entry->Control_Flags & fBND_STRG_CLIENT_SUPPORT_H5G) 
			{
				...
				ac_connectednum = bndstrg_get_connectnum(bndstrg,BAND_5G_L|BAND_5G_H); 
                                ac_mbssmaxnum = table->AC_MbssMaxNumLimit;				
                                //ac_mbssmaxnum = bndstrg_get_maxstanum_perssid(bndstrg,BAND_5G_L|BAND_5G_H);
				BND_STRG_PRINTQAMSG(table,entry,"++++H5G_ONLY++Addr=%02x:%02x:%02x:%02x:%02x:%02x,G5_ConnectedNum:%d,G5_ConnectedlimitNum:%d\n\r",
						PRINT_MAC(entry->Addr),ac_connectednum,ac_mbssmaxnum);
				if(ac_connectednum >= ac_mbssmaxnum)
					band = BAND_2G;
				else
            ...
            else 
			{
				...
				//ac_mbssmaxnum = bndstrg_get_maxstanum_perssid(bndstrg,BAND_5G_L|BAND_5G_H);
                                ac_mbssmaxnum = table->AC_MbssMaxNumLimit;				
                                BND_STRG_PRINTQAMSG(table,entry,"++++L5G_ONLY++Addr=%02x:%02x:%02x:%02x:%02x:%02x,G5_ConnectedNum:%d,G5_ConnectedlimitNum:%d\n\r",
						PRINT_MAC(entry->Addr),ac_connectednum,ac_mbssmaxnum);
				if(ac_connectednum >= ac_mbssmaxnum)
					band = BAND_2G;
				else
...
u8 bndstrg_steer_sta(struct bndstrg *bndstrg, struct bndstrg_cli_entry *entry)
{
	...
	if((table->BtmMode & BTM_Only) && (!entry->btm_info.BTMSupport))
		return FALSE;
#ifdef TCSUPPORT_WLAN_BNDSTRG_CUSTOM1	
	BND_STRG_PRINTQAMSG(table, entry,("Addr=%02x:%02x:%02x:%02x:%02x:%02x entry->Operation_steered ===== %d\n"),
		PRINT_MAC(entry->Addr),entry->Operation_steered);//add by yp
#endif
	if (entry->Operation_steered == TRUE)
	{
		entry->Operation_steered_tick++;
...
#endif
#ifdef TCSUPPORT_WLAN_BNDSTRG_CUSTOM1
		BND_STRG_PRINTQAMSG(table, entry,("Addr=%02x:%02x:%02x:%02x:%02x:%02x POST_STEER ===== START\n"),
					PRINT_MAC(entry->Addr));//add by yp
#endif
...
#ifdef TCSUPPORT_WLAN_BNDSTRG_CUSTOM2
//#ifdef VENDOR_FEATURE7_SUPPORT  //FPT 0615 modify
			if(entry->BlackListTimer > 0){
BND_STRG_PRINTQAMSG(table, entry,GRN("%s[%d]BlackListTimer:%d, \n"),
	__func__,__LINE__, entry->BlackListTimer);			
...
#ifdef VENDOR_IOS_SUPPORT
			//20180309, For IOS immediately connect
...
#endif
		if (BndStrgGetKeyParameter("RssiWorst", keyvalue, buf)) {
			table->RssiWorst = strtol(keyvalue, NULL, 10);
		}
		fclose(fp);
	}
	return;
...
int bndstrg_table_init(struct bndstrg_cli_table *table)
{
	...
	if (table->bInitialized == TRUE)
		return BND_STRG_SUCCESS;
	printf("%s() - table = %p, and size = %d\n", __func__, table, sizeof(struct bndstrg_cli_table));
	memset(table, 0, sizeof(struct bndstrg_cli_table));
	printf("%s() - table = %p cleared\n", __func__, table);
	table->Band = 0;
	table->bEnabled = 0;
	table->RssiDiff= BND_STRG_RSSI_DIFF;
	table->RssiLow = BND_STRG_RSSI_LOW;
	table->RssiWorst = BND_STRG_RSSI_WORST;
	table->RSSILowDownSteer = BND_STRG_DOWNSTEER_RSSI;
	table->RSSIHighUpSteer = BND_STRG_UPSTEER_RSSI;
#ifdef TCSUPPORT_WLAN_BNDSTRG_JOYME3
	table->DetectInterval = 30; // added by huaming
	table->IdleThreshold  = IDLE_RXTX_BYTE_COUNT;// added by huaming
#endif
#ifdef VENDOR_FEATURE7_SUPPORT
	...
	BndStrg_Update_Default_Param(table);
	{
		...
		for(i=0; i < table->PriorityListSize; i++)
		{
			...
			(table->PriorityList[i] == fBND_STRG_PRIORITY_DEFAULT_2G)){
				bDefault_set = TRUE;
			} else if((table->PriorityList[i] == fBND_STRG_PRIORITY_FAST_FORWARD))
				table->fFastForwardAlgo = TRUE;
			else
				table->BndStrgMode |= POST_CONNECTION_STEERING;
				 ...
				 int version)
{
	...
	DBGPRINT(DEBUG_OFF,GRN("bndstrg_table_init() done ::bndstrg=%p,table=%p\n\r"), bndstrg,&bndstrg->table);
#ifdef BNDSTRG_NVRAM_SUPPORT
...
void bndstrg_inf_init(struct bndstrg_iface *inf)
{
	if(IS_5G_H_BAND(inf->Band)){
		...
		#ifdef LIMIT_5G_NUM_SUPPORT
		if(inf->ucIfName[0] == 'r' && inf->ucIfName[1] == 'a'&& inf->ucIfName[2] == 'i')
			inf->wifiindex = inf->ucIfName[3] -'0';
		#endif
	}
	...
	if(IS_2G_BAND(inf->Band)){
		...
		#ifdef LIMIT_5G_NUM_SUPPORT
		if(inf->ucIfName[0] == 'r' && inf->ucIfName[1] == 'a')
			inf->wifiindex = inf->ucIfName[2]- '0' ;
		#endif
	}
	return;
}
...
count=0;
	...
	if(!entry){
		...
		if(entry_index >= BND_STRG_MAX_TABLE_SIZE) {
			BND_STRG_DBGPRINT(DEBUG_OFF,
				"\t%s(): cannot find old entry to delete", __func__);
			return NULL;
		}
		entry = &table->Entry[entry_index];
#else
...
void bndstrg_cli_status_rsp (struct bndstrg *bndstrg, struct bnd_msg_cli_status_rsp *cli_status_rsp)
{
	...
	//printf("%s() - before Tx avg = %d, index = %d\n", __func__, data_tx_byte->avg, data_tx_byte->idx);
	/*
	...
	*/
	...
	data_tx_byte->avg = 0;
	for (idx = 0; idx < BND_STRG_DATA_SAMPLE; idx++)
		data_tx_byte->avg += data_tx_byte->data[idx];
	data_tx_byte->avg /= BND_STRG_DATA_SAMPLE;
	INC(data_tx_byte->idx,BND_STRG_DATA_SAMPLE);
	//printf("%s() - after  Tx avg = %d, index = %d\n", __func__, data_tx_byte->avg, data_tx_byte->idx);

	//printf("%s() - before Rx avg = %d, index = %d\n", __func__, data_rx_byte->avg, data_rx_byte->idx);
	/*
	...
	*/
	...
	data_rx_byte->avg = 0;
	for (idx = 0; idx < BND_STRG_DATA_SAMPLE; idx++)
		data_rx_byte->avg += data_rx_byte->data[idx];
	data_rx_byte->avg /= BND_STRG_DATA_SAMPLE;
	INC(data_rx_byte->idx,BND_STRG_DATA_SAMPLE);
	//printf("%s() - after  Rx avg = %d, index = %d\n\n", __func__, data_rx_byte->avg, data_rx_byte->idx);
