		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				...
				if (packet_num < 3) {
...
#if !defined(TCSUPPORT_CT_FJ) && !defined(TCSUPPORT_CT_DHCP_FORCERENEW) && !defined(TCSUPPORT_CT_PON_SC) 
					...
					packet_num = 0;
#endif
#if !defined(TCSUPPORT_CT_FJ) && !defined(TCSUPPORT_CT_DHCP_FORCERENEW) && !defined(TCSUPPORT_CT_PON_SC) && !defined(TCSUPPORT_CT_PON_JX) 
					timeout = now + 60;
#endif
				}
				break;
