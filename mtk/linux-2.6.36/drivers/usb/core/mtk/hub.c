...
/* Patch for USB Led, yuanshang, 2011-11-30 */
/* Attention: USB leds only for root hub */
USB_LED_HANDLE *usbLedTbl = NULL;
unsigned int usbLedNum = 0;
unsigned int is3g = 0;
	...
	struct usb_endpoint_descriptor *endpoint)
{
  ...
  if (hub_is_superspeed(hdev) && (hdev->parent != NULL)) {
	...
	/* Patch for USB Led, yuanshang, 2011-11-30 */
	/* Init usbLedTbl, NOTE: not free */
	if ((NULL == usbLedTbl) && (0 == hdev->level))
	{
		
		usbLedNum = hdev->maxchild;
		usbLedTbl = kmalloc((sizeof(USB_LED_HANDLE) * usbLedNum), GFP_KERNEL);
		if (NULL == usbLedTbl)
		{
			dev_info (hub_dev, "can't kmalloc usb hub's usbLedTbl\n");
		}
		else
		{
			memset(usbLedTbl, 0, (sizeof(USB_LED_HANDLE) * usbLedNum));
		}
	}
					...
					u16 portstatus, u16 portchange)
{
	...
	/* Transplanted by zjj, 20130530.(Patch for USB Led, yuanshang, 2011-11-30) */
	
	if ((NULL != usbLedTbl) && (0 == hdev->level) && (usbLedNum >= port1))
	{
		if (!(portstatus & USB_PORT_STAT_CONNECTION))
		{
			if (usbLedTbl[port1-1].fpLetOff)
				usbLedTbl[port1-1].fpLetOff(usbLedTbl[port1-1].arg);
			else
				usbLedTbl[port1-1].portStats = 0;
		}
		else
		{
			if (usbLedTbl[port1-1].fpLetOn)
				usbLedTbl[port1-1].fpLetOn(usbLedTbl[port1-1].arg);
			else
				usbLedTbl[port1-1].portStats = 1;
		}
	}
...
EXPORT_SYMBOL(usbLedTbl);
EXPORT_SYMBOL(usbLedNum);
EXPORT_SYMBOL(is3g);
