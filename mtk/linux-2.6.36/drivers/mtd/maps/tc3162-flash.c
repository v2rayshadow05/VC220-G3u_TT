...
#include "../../global_inc/flash_api.h"
...
// roy add 2017 06 22
#ifndef INCLUDE_DUAL_IMAGE

#define ROOTFS_SLAVE_PART "rootfs_slave"

#else
#define ROOTFS_SLAVE_PART "rootfs_2"
#endif
...
#elif defined(INCLUDE_FLASH_16M)
static struct mtd_partition tc3162_parts[] = {
	{									 	/* 1th partition */
		  name 	     : "boot",	 	        /* Bootloader section */ 
		  size	  	 : 0x00020000, 		 	/* Size  */
...
#elif defined(INCLUDE_FLASH_128M)
struct mtd_partition tc3162_parts[] = {
	...
	{									 	/* 2th partition */
		  ...
		  offset     : BOOT_SIZE			/* Append after bootloader section */
	},
	{									 	/* 3th partition */
		  name       : "rootfs", 		 	/* Root filesystem section */
		  size	     : ROOTFS_SIZE,	        /* Size */
		  offset     : BOOT_SIZE +  KERNEL_SIZE /* Append after kernel section */
	},
	{									 	/* 4th partition */
		  name       : "ker_rofs", 		 	/* kernel + root filesystem section */
		  size	     : ROOTFS_SIZE + KERNEL_SIZE,	        /* Size */
		  offset     : BOOT_SIZE 			/* Append after boot section */
	},
#ifdef INCLUDE_DUAL_IMAGE
	{									 	/* 5th partition */
		  name       : "kernel_2",		 	/* Kernel 2 section */
		  size		 : KERNEL_SIZE,			/* Size */
		  offset     : KERNEL_2_OFFSET		/* Append after root filesystem section */
	},
	{									 	/* 6th partition */
		  name       : "rootfs_2", 		 	/* Root filesystem 2 section */
		  size	     : ROOTFS_SIZE,	        /* Size */
		  offset     : KERNEL_2_OFFSET +  KERNEL_SIZE		/* Append after kernel 2 section */
	},
	{									 	/* 7th partition */
		  name       : "ker_rofs_2", 		/* kernel 2 + root filesystem 2 section */
		  size	     : ROOTFS_SIZE + KERNEL_SIZE,	        /* Size */
		  offset     : KERNEL_2_OFFSET 			/* Append after root filesystem section */
	},
#endif
    {                                       /* 8th partition */
          name       : "romfile",           /* config filesystem section */
          size       : ROOM_FILE_SIZE,			/* Size 0x20000*/
          offset     : ROOM_FILE_OFFSET   		/* Append after Root filesystem 2 section  offset 0x06420000 */
    },
	{ 									    /* 9th partition */
		  name	     : "config",			/* romfile filesystem section */
		  size	     : CONFIG_SIZE,	        /* Size 0x20000*/
		  offset	 : CONFIG_OFFSET   		/* Append after romfile section offset 0x06440000*/
	},
	{ 									    /* 10th partition */
		  name	     : "radio",			    /* Radio section */
		  size       : RADIO_FILE_SIZE,    		/* Size 0x00020000*/
		  offset	 : RADIO_OFFSET   		/* Append after config section offset 0x06460000*/
	},
	{ 									    /* 11th partition */ 
		  name	     : "reserve_area",		/* reserve_area section */ 
		  size       : MTDPART_SIZ_FULL,    		/* Size */
		  offset	 : RESERVE_AREA_OFFSET   /* Append after radio section offset 0x06480000*/ 
	}
#else
	...
	{									 	/* 2th partition */
		  name       : "romfile",		 	/* Kernel section */
		  size		 : ROOM_FILE_SIZE,		/* Size  */
		  offset     : BOOT_SIZE			/* Append after bootloader section */
	},
	{									 	/* 3th partition */
		  name       : "config", 		 	/* Root filesystem section */
		  size	     : CONFIG_SIZE,	        /* Size */
		  offset     : CONFIG_OFFSET /* Append after kernel section */
	},
	{ 									    /* 10th partition */
		  name	     : "radio",			    /* Radio section */
		  size       : RADIO_FILE_SIZE,    		/* Size 0x00020000*/
	...
	{									 	/* 2th partition */
		  name       : "kernel",		 	/* Kernel section */
		  size		 : KERNEL_SIZE,		/* Size  */
		  offset     : KERNEL_OFFSET			/* Append after bootloader section */
	},
	{									 	/* 3th partition */
		  name       : "rootfs", 		 	/* Root filesystem section */
		  size	     : ROOTFS_SIZE,	        /* Size */
		  offset     : KERNEL_OFFSET +  KERNEL_SIZE /* Append after kernel section */
	},
	{									 	/* 4th partition */
		  name       : "ker_rofs", 		 	/* kernel + root filesystem section */
		  size	     : ROOTFS_SIZE + KERNEL_SIZE,	        /* Size */
		  offset     : KERNEL_OFFSET 			/* Append after boot section */
	},
#ifdef INCLUDE_DUAL_IMAGE
	{									 	/* 5th partition */
		  name       : "kernel_2",		 	/* Kernel 2 section */
...
static void tc3162_put_rootfs(void){
	...
	/*INCLUDE_DUAL_IMAGE defined in config.in,congigure by menuconfig
	 * BOOT_FLAG_ADDR defiend in ../../global_inc/flash_api.h
	 */
#if defined(INCLUDE_DUAL_IMAGE)
	char *bufaddr = (char*)BOOT_FLAG_ADDR;
	const char flagvalue = 1;//not change!!because we use this flag to judge which image
	char tmp[8] = {0};
	u_char bootflag_char = 0;
	spiflash_ioctl_read(&bootflag_char,bufaddr,1);
	tmp[0] = bootflag_char - '0';
...
// roy add 2017 06 14
	...
	int index_int = 0;
	#if 0
	tc3162_parts[index_int].offset = 0x00000000; // boot
	tc3162_parts[index_int].size =   BOOT_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = BOOT_SIZE; // kernel
	tc3162_parts[index_int].size =   KERNEL_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = KERNEL_SIZE + BOOT_SIZE; // rootfs
	tc3162_parts[index_int].size =   ROOTFS_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = BOOT_SIZE; // ker_rofs
	tc3162_parts[index_int].size =   KERNEL_SIZE + ROOTFS_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = KERNEL_2_OFFSET; // kernel_2
	tc3162_parts[index_int].size =   KERNEL_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = KERNEL_2_OFFSET + KERNEL_SIZE; // rootfs_2
	tc3162_parts[index_int].size =   ROOTFS_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = KERNEL_2_OFFSET; // ker_rofs_2
	tc3162_parts[index_int].size =   ROOTFS_SIZE + KERNEL_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = ROOM_FILE_OFFSET; // romfile
	tc3162_parts[index_int].size =   ROOM_FILE_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = CONFIG_OFFSET; // config
	tc3162_parts[index_int].size =   CONFIG_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = RADIO_OFFSET; // radio
	tc3162_parts[index_int].size =   RADIO_FILE_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = RESERVE_AREA_OFFSET; // reserve_area
	tc3162_parts[index_int].size =   RESERVE_AREA_SIZE;
	#else
	tc3162_parts[index_int].offset = 0x00000000; // boot
	tc3162_parts[index_int].size =   BOOT_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = BOOT_SIZE; // rom
	tc3162_parts[index_int].size =   ROOM_FILE_SIZE;
	index_int++;
	tc3162_parts[index_int].offset =CONFIG_OFFSET; // config
	tc3162_parts[index_int].size =   CONFIG_SIZE;
	...
	tc3162_parts[index_int].size =   RADIO_FILE_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = KERNEL_OFFSET; // kernel_1
	tc3162_parts[index_int].size =   KERNEL_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = KERNEL_OFFSET+KERNEL_SIZE; // rootfs_1
	tc3162_parts[index_int].size =   ROOTFS_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = KERNEL_OFFSET; // ker_rofs_1
	tc3162_parts[index_int].size =   ROOTFS_SIZE + KERNEL_SIZE;
	index_int++;
	tc3162_parts[index_int].offset = KERNEL_2_OFFSET; // kernel_2
	tc3162_parts[index_int].size =   KERNEL_SIZE;
