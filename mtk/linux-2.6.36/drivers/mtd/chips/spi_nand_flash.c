...
{		
	...
	if( spi_nand_block_aligned_check( addr, len) == SPI_NAND_FLASH_RTN_NO_ERROR)
	{
		...
		while( erase_len < len )
		{
			...
			{
#if	defined(TCSUPPORT_NAND_BMT) && !defined(LZMA_IMG)
					/* Add by YuChuwei-20180302
					Pathc A20170100_EN751xEN752x_SPI_NAND_OSBNB00073929_ TLM7 3 246 0-194_20171130.
					Fix Bug: For EN751x/EN752x SPI NAND, if some blocks are bad and some remapping blocks of BMT Pool are bad too, data area will be demaged.
					*/
#if 0
...
static void spi_nand_manufacute_init( struct SPI_NAND_FLASH_INFO_T *ptr_device_t )
{
	...
	if((ptr_device_t->mfr_id) == _SPI_NAND_MANUFACTURER_ID_MXIC)
	{
		/* 1. Unlock All block */
		spi_nand_protocol_get_status_reg_1(&feature);
		feature &= 0xC1;
			...
			((ptr_device_t->mfr_id == _SPI_NAND_MANUFACTURER_ID_ESMT) && (ptr_device_t->dev_id == _SPI_NAND_DEVICE_ID_F50L1G41LB)))
	{
		/* 1. Unlock All block */
		spi_nand_protocol_get_status_reg_1(&feature);
		if(ptr_device_t->dev_id == _SPI_NAND_DEVICE_ID_F50L1G41LB) {
			feature = 0x0;
		} else {
			feature &= 0xC7;
		}
...
#if (defined(CONFIG_PW_MODEL_VMG1312T20AV1) || defined(CONFIG_PW_MODEL_VMG1302T20AV1))
...
#elif defined(INCLUDE_DUAL_IMAGE)
char *l_p_mtd_name[] = {"boot", "romfile", "config","radio","kernel","rootfs", "ker_rofs","kernel_2","rootfs_2","ker_rofs_2", "reserve_area" ,NULL};
#define NP_TOTAL            11
...
#endif
		//printk("(u32)opt->dest :0x%x,opt->bytes is :0x%x",(u32)opt->dest,opt->bytes);
...
{	
...
#if defined(CONFIG_PW_MODEL_PVW41T300NV2)
	...
	printk("[0x%x]:0x%02x%02x\n",RADIO_2G_FLASH_START,calData_2g[0],calData_2g[1]);
	printk("[0x%x]:0x%02x%02x\n",RADIO_5G_FLASH_START,calData_5g[0],calData_5g[1]);
	if (((0xff == calData_2g[0] && 0xff == calData_2g[1]) 
		  || (0x0 == calData_2g[0] && 0x0 == calData_2g[1]))
