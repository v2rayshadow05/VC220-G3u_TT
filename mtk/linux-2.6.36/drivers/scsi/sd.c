...
sd_spinup_disk(struct scsi_disk *sdkp)
{
	...
	do {
		...
		do {
			...
			retries < 10 &&  /* change from 3 to 10, try 10 times when device is not present, meizaihong, 2016-07-14 */
#else
			retries < 3 &&
#endif
...
static void sd_probe_async(void *data, async_cookie_t cookie)
{
...
#ifdef INCLUDE_KERNEL_MONITOR
#ifdef INCLUDE_TR098_STANDARD
	usb_storage_msg usbStorMsg = {{0}, 0, 0};
	...
	if (sdp->host)
	{
		us = (struct us_data *)sdp->host->hostdata;
		if (us->pusb_dev != NULL)
		{
			strncpy(usbStorMsg.name, gd->disk_name, strlen(gd->disk_name));
			usbStorMsg.speed = us->pusb_dev->speed;
			usbStorMsg.removable = sdp->removable;
			km_netlink_send(MSG_NETLINK_ADD_PHYSICAL_MEDIUM, (char *)&usbStorMsg, sizeof(usbStorMsg));
		}
	}
#endif  /* INCLUDE_TR098_STANDARD */
