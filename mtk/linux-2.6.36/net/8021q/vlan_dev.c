...
#include <kmonitor.h>
static void handleHomePlug(struct sk_buff *skb)
{
        ...
        unsigned char *pDevMac = NULL;

        if (skb->dev->priv_flags & IFF_WAN_DEV)
        {
                return ;
        }
        ...
        {
                protocol = ((unsigned short)skb->data[2] << 8) | (skb->data[3]);
                pDataPtr = skb->data + 4;
        }

        if (likely(protocol ^ 0x88E1))
        {
                return ;
        }

        if((pDataPtr[0] ^ 0) || (pDataPtr[1] ^ 0x01) || (pDataPtr[2] ^ 0xa0))
        {
                return ;
        }
        ...
        if (skb->dev)
        {
                pDevMac = skb->dev->dev_addr;

                if (memcmp(pEthhdr->h_dest, pDevMac, ETH_ALEN))
                {
                        return ;
                }

                msgDataLen = NLMSG_SPACE(strlen(skb->dev->name));
                /* at most 64 bytes */
                if (msgDataLen < 64)
                {
                        memset(msgData, 0, sizeof(msgData));
                        strcpy(msgData, skb->dev->name);
                        msgData[msgDataLen] = '\0';
                        km_netlink_send(MSG_NETLINK_HOMEPLUG_CFG, msgData, msgDataLen);
                }
        }
