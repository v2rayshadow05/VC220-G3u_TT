			      ...
			      struct net_device *orig_dev)
{
    ...
    (defined(CONFIG_ATM_LANE) || defined(CONFIG_ATM_LANE_MODULE) || defined(CONFIG_X_TP_VLAN))
...
static int  check_dev_ip_addr(struct net_device *dev, unsigned int addr)
{
...
ncls:
	...
	if(skb->protocol == cpu_to_be16(ETH_P_ARP))
	{
		...
		if(arp->ar_op == ARPOP_REPLY)
		{
			...
			if(check_dev_ip_addr(skb->dev,ntohl(sip)) == 0)
			{
				printk("maybe some dev is imposing a gateway\n");
				kfree_skb(skb);
				return NET_RX_DROP;
			}
		}
...
#if defined(TCSUPPORT_DEV_EXTSTATS)
...
static void devextstats_seq_printf_stats(struct seq_file *seq, struct net_device *dev)
{
	...
	if((stats->multicast + stats->rx_broadcast_packets) < stats->rx_packets)
		...
		rx_unicast_packets = stats->rx_unicast_packets;
	...
	if((stats->tx_multicast_packets + stats->tx_broadcast_packets) < stats->tx_packets)
		...
		tx_unicast_packets = stats->tx_unicast_packets;
	...
	while (atomic_read(&dev->refcnt) != 0) {
		...
		if (time_after(jiffies, warning_time + 10 * HZ)) {
			...
			if(atomic_read(&dev->refcnt) != 0 && (dev->name[0]=='n'
							...
							|| dev->name[0]=='r'
							|| dev->name[0]=='b') //for bridge brx
							|| dev->name[0]=='l')//for lte
			{
