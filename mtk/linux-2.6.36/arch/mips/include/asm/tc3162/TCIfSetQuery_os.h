...
#define         ADSL_QUERY_PMS_INP                      0x0036
#define         VDSL_QUERY_TR098_DSL_DIAGNOSTIC   		0x2002
...
#define         VDSL_QUERY_BAND_PLAN   					0x2006
#define         ADSL_QUERY_USBIT  						0x0037
#define         ADSL_QUERY_DSBIT  						0x0038

#define 		HLOG_INVALID_VALUE						(1023)
#define			QLN_INVALID_VALUE						(255)
#define			SNR_INVALID_VALUE						(255)
#define			SNRM_MIN_INVALID_VALUE						(-511)
#define			SNRM_MAX_INVALID_VALUE						(511)
#define 		SATN_INVALID_VALUE						(1023)
#define 		LATN_INVALID_VALUE						(1023)

typedef struct
{
   unsigned short HLOGpsus_vdsl[512];//上行HLOG
   short HLINpsus_vdsl[512][2];
   unsigned char  QLNpsus_vdsl[512];//上行QLN
   unsigned char  SNRpsus_vdsl[512];//上行SNR
   unsigned short SATNds_vdsl[5];
   unsigned short SATNus_vdsl[5];
   unsigned short LATNpbds_vdsl[5];
   unsigned short LATNpbus_vdsl[5];
}TR098_WAN_VDSL_DIAGNOSTIC;

typedef struct
{
   unsigned short HLINSCus;
   unsigned char HLINGds;
   unsigned char HLINGus;
   unsigned char HLOGGds;//下行HLOG一个group代表几个tone
   unsigned char HLOGGus;//上行HLOG一个group代表几个tone
   unsigned short  HLOGpsds[512];//下行HLOG
   unsigned short  HLOGpsus[64];
   unsigned short  HLOGMTds;
   unsigned short  HLOGMTus;
   unsigned short  SATNds;
   unsigned short  SATNus;
   short  HLINpsus[64][2];
   unsigned char QLNGds;//下行QLN一个group代表几个tone
   unsigned char QLNGus;//上行QLN一个group代表几个tone
   unsigned char QLNpsus[64];
   unsigned short  QLNMTds;
   unsigned short  QLNMTus;
   unsigned char SNRGds;//下行SNR一个group代表几个tone
   unsigned char SNRGus;//上行SNR一个group代表几个tone
   unsigned char SNRpsus[64];
   unsigned short SNRMTds;
   unsigned short SNRMTus;
   unsigned char BITSpsus[64];
}TR098_WAN_DSL_DIAGNOSTICS_2;
typedef struct
{
   unsigned char DiagnosticState[16];
   short ACTPSDds;
   short ACTPSDus;
   short ACTATPds;
   short ACTATPus;
   unsigned short  HLINSCds;
   short  HLINpsds[512][2];
   unsigned char  QLNpsds[512];//下行QLN
   unsigned char  SNRpsds[512];//下行SNR
   unsigned char  BITSpsds[512];
   unsigned short  GAINSpsds[512];
}TR098_WAN_DSL_DIAGNOSTICS;
typedef struct
{
   unsigned short period_us[64];//上行一个band几个tone
   unsigned short start_idx_us[64];//上行band起始tone
   unsigned short end_idx_us[64];//上行band结束tone
   unsigned short period_ds[64];//下行一个band几个tone
   unsigned short start_idx_ds[64];//下行band起始tone
   unsigned short end_idx_ds[64];//下行band结束tone
}WAN_VDSL_BAND_PLAN;

typedef struct
{
  unsigned char CurrentProfiles;
  unsigned short UPBOKLE;
  unsigned char TRELLISds;
  unsigned char TRELLISus;
  unsigned char ACTSNRMODEds;
  unsigned char ACTSNRMODEus;
  unsigned short ACTUALCE;
  short SNRMpbds[5];
  short SNRMpbus[5];
...
union DSL_PARAM{
	TR098_WAN_VDSL_DIAGNOSTIC vd_diag;
	TR098_WAN_DSL_DIAGNOSTICS_2 dsl_diag2;
	TR098_WAN_DSL_DIAGNOSTICS dsl_diag;
	WAN_VDSL_BAND_PLAN bandPlan;
	TR098_VDSL_INTERFACE_CFG intfCfg;
	PMS_PARAMS pmsParam;
	unsigned char  BPTus[4096];//表示第tone_idx的BITS
	unsigned char  BPTds[4096];//表示第tone_idx的BITS
};
